...
public abstract class AbstractSourceTree extends JTree {
	...
	/**
	 * Expands all nodes in the specified tree. Subclasses should implement this in
	 * a way logical for the language.
	 */
	public abstract void expandInitialNodes();

	/**
	 * An attempt to quickly expand all tree nodes. Only need to expand the
	 * "deepest" nodes, as they will auto-expand all parents to make sure they are
	 * visible.
	 *
	...
	public void filter(String pattern) {
		if ((pattern == null && this.pattern != null) || (pattern != null && this.pattern == null)
	 ...
	 * tree. An example of a "minor" element could be a local variable in a function
	 * or method.
	 *
	...
	public void selectFirstNodeMatchingFilter() {
		...
		while (en.hasMoreElements()) {
			...
			JLabel renderer = (JLabel) getCellRenderer().getTreeCellRendererComponent(this, stn, true, true,
					stn.isLeaf(), 0, true);
	...
	/**
	 * Sets whether, when a source element is selected in this tree, the same source
	 * element should be selected in the editor.
	 *
	 ...
	 * source tree. An example of a "minor" element could be a local variable in a
	 * function or method.
	 *
