...
public class JarManager {
	...
	/**
	 * Whether to check datestamps on jars/directories when completion information
	 * is requested.
	 */
	...
	public void addCompletions(CompletionProvider p, String text, Set<Completion> addTo) {
		/*
		 * The commented-out code below is probably replaced by the rest of the code in
		 * this method... TODO: Verify me!!! // Add any completions matching the text
		 * for each jar we know about String[] pkgNames = Util.splitOnChar(text, '.');
		 * for (int i=0; i<jars.size(); i++) { JarReader jar = (JarReader)jars.get(i);
	...
	/**
	 * Adds a jar to read from. This is a convenience method for folks only reading
	 * classes from jar files.
	 *
	 * @param jarFile The jar to add. This cannot be <code>null</code>.
	 * @return Whether this jar was added (e.g. it wasn't already loaded, or it has
	 *         a new source path).
	 ...
	 *
	 * @param info The source to add. If this is <code>null</code>, then the current
	 ...
	 *             location).
	 * @return Whether this source was added (e.g. it wasn't already loaded, or it
	...
	/**
	 * Adds the current JVM's rt.jar (or class.jar if on OS X) to the list of class
	 * file sources. If the application is running in a JDK, the associated source
	 * zip is also located and used.
	 *
	 * @throws IOException If an IO error occurs.
	 * @see #addClassFileSource(LibraryInfo)
	 */
	...
	/**
	 * Returns whether the "last modified" time stamp on jars and class directories
	 * should be checked whenever completions are requested, and if the
	 * jar/directory has been modified since the last time, reload any cached class
	 * file data. This allows for code completion to update whenever dependencies
	 * are rebuilt, but has the side effect of increased file I/O. By default this
	 * option is enabled; if you somehow find the file I/O to be a bottleneck
	 * (perhaps accessing jars over a slow NFS mount), you can disable this option.
	 *
	 * @return Whether jars/directories are checked for modification since the last
	 ...
	 * @return A list of type declarations with the given name, or <code>null</code>
	 *         if there are none.
	 */
	 ...
	 *
	 * @return A list of {@link LibraryInfo}s. Modifying a <code>LibraryInfo</code>
	 *         in this list will have no effect on this completion provider; in
	 *         order to do that, you must re-add the jar via
	 *         {@link #addClassFileSource(LibraryInfo)}. If there are no jars on the
	...
	/**
	 * Removes a jar from the "build path." This is a convenience method for folks
	 * only adding and removing jar sources.
	 *
	 * @param jar The jar to remove.
	 * @return Whether the jar was removed. This will be <code>false</code> if the
	 *         jar was not on the build path.
	 * @see #removeClassFileSource(LibraryInfo)
	 * @see #addClassFileSource(LibraryInfo)
	 * @see #getClassFileSources()
	 */
	 ...
	 * @return Whether source jar was removed. This will be <code>false</code> if
	 *         the source was not on the build path.
	 * @see #removeClassFileSource(File)
	 * @see #addClassFileSource(LibraryInfo)
	 * @see #getClassFileSources()
	 */
	 ...
	 *              since the last access, and clear any cached completion
	 *              information if so.
