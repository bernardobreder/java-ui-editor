 ...
 * large Java classes. Using Swing's HTML support is simply too slow (see
 * {@link CompletionCellRenderer}).
 * <p>
...
public class JavaCellRenderer extends DefaultListCellRenderer {
	...
	/**
	 * This is used instead of jsc for "incomplete" stuff, like classes, interfaces,
	 * etc. read from jars (don't yet read the class in, for example).
	 */
	private Completion nonJavaCompletion;

	/**
	 * Whether to not display extra info (type, etc.)in completion text, just the
	 * completion's name. The default is <code>false</code>.
	 */
	...
	/**
	 * Sets whether to display "simple" text about the completion - just the name,
	 * no type information, etc. The default value is <code>false</code>.
	 *
