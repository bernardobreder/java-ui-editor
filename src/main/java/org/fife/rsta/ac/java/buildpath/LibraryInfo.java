...
public abstract class LibraryInfo implements Comparable, Cloneable {
	...
	/**
	 * The location of the source files corresponding to this library. This may be
	 * <code>null</code> .
	 */
	 ...
	 * {@link #createClassFileBulk(String)}. After calling this method, the actual
	 * class file fetching should be done in a try/finally block that ensures a call
	 * to {@link #bulkClassFileCreationEnd()}; e.g.
	 *
	 * <pre>
	 ...
	 * individual classes and shove it into their package maps.
	 * <p>
	 * This method should be used when multiple classes will be fetched from this
	 * library at the same time. It should only be called after a call to
	 * {@link #bulkClassFileCreationStart()}. If only a single class file is being
	 * fetched, it is simpler to call {@link #createClassFile(String)}.
	 *
	 * @param entryName The fully qualified name of the class file.
	 * @return The class file, or <code>null</code> if it isn't found in this
	 *         library.
	...
	/**
	 * Two <code>LibraryInfo</code>s are considered equal if they represent the same
	 * class file location. Source attachment is irrelevant.
	 *
	 * @return Whether the specified instance represents the same class source as
	 *         this one.
	 */
	@Override
	...
	/**
	 * Returns information on the "main" jar for a JRE. This will be <tt>rt.jar</tt>
	 * everywhere except OS X, where it will be <tt>classes.jar</tt>. The associated
	 * source zip/jar file is also checked for.
	 *
	...
	/**
	 * Returns the time this library was last modified. For jar files, this would be
	 * the modified date of the file. For directories, this would be the time a file
	 * in the directory was most recently modified. This information is used to
	 * determine whether callers should clear their cached package map information
	 * and load it anew.
	 * <p>
	 ...
	 * contained in a single jar file, this will be the full path to that jar. If it
	 * is a directory containing classes, it will be the full path of the directory.
	 * Otherwise, this value will be <code>null</code>.
	 *
	 ...
	 * overridden. Instances of <code>LibraryInfo</code> aren't typically stored in
	 * maps, so the hash value isn't necessarily important to
