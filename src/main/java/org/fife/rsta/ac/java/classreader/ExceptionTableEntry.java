...
public class ExceptionTableEntry {
	 ...
	 * This value must be either a valid index into the code array of the opcode of
	 * an instruction, or be equal to {@link Code#getCodeLength()}.
	 */
	private int endPC; // u2

	/**
	 * The start of the exception handler. This value must be a valid index into the
	 * code array and must be the index of the opcode of an instruction.
	 */
	private int handlerPC; // u2

	/**
	 * If the value of {@link #catchType} is nonzero, it must be a valid index into
	 * the constant pool. The constant pool entry at that index must be a
	 * {@link ConstantClassInfo} structure representing a class of exceptions that
	 * this exception handler is designated to catch. This class must be the class
	 * <code>Throwable</code> or one of its subclasses. The exception handler will
	 * be called only if the thrown exception is an instance of the given class or
	 * one of its subclasses.
	 * <p>
	 * If the value of {@link #catchType} is zero, this exception handler is for all
	 * exceptions. This is used to implement <code>finally</code>.
	 */
	...
	/**
	 * Returns the name of the <tt>Throwable</tt> type caught and handled by this
	 * exception handler.
	 *
	 * @param fullyQualified Whether the name should be fully qualified.
	 * @return The name of the <tt>Throwable</tt> type, or <code>null</code> if this
	 *         entry denotes a <tt>finally</tt> block.
	 */
