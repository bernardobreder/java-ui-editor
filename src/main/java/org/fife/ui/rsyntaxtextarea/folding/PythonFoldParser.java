...
/**
 * A fold parser for Python. Analyzes indentation to determine foldable regions.
 * Lines containing only comments and/or whitespace are ignored.
 *
		...
		try {
			...
			for (int line = 0; line < lineCount; line++) {
				...
				if (leadingWhiteSpaceCount == currentLeadingWhiteSpaceCount) {
					...
					if (currentFold != null) {
						currentFold = currentFold.createChild(FoldType.CODE, currentNextFoldStart);
					} else {
