...
public abstract class RPrintUtilities {
	 ...
	 * <code>maxCharsPerLine-1</code> if none of the <code>breakChars</code> comes before that
	 * position. This position represents the logical line break for this
	 * <code>java.lang.String</code> if it is being printed in a monospaced font when lines can only
	 * be <code>maxCharsPerLine</code> characters long.
	 *
	 ...
	 *            on one line.
	 * @return The logical position at which to stop printing <code>line</code> to simulate word
	 *         wrap.
	 */
	...
	public static int printDocumentMonospaced(Graphics g, Document doc, int fontSize, int pageIndex, PageFormat pageFormat, int tabSize) {
		...
		while (currentDocLineNumber < numDocLines) {
			...
			// Get rid of newlines, because they end up as boxes if you don't; this is a monospaced
			// font.
		...
		// Now, the whole document has been "printed." Decide if this page had any text on it or
		// not.
		if (numPrintedLines > STARTING_LINE_NUMBER) {
			return Printable.PAGE_EXISTS;
		}
	...
	public static int printDocumentMonospacedWordWrap(Graphics g, Document doc, int fontSize, int pageIndex, PageFormat pageFormat, int tabSize) {
		...
		while (currentDocLineNumber < numDocLines) {
			...
			// Remove newlines, because they end up as boxes if you don't; this is a monospaced
			// font.
			curLineString = curLineString.replaceAll("\n", "");
	...
	public static int printDocumentWordWrap(Graphics g, JTextComponent textComponent, Font font, int pageIndex, PageFormat pageFormat, int tabSize) {
		...
		while (currentDocLineNumber < numDocLines) {
			...
			// (because this line may be the second part of a wrapped line, so we'd start after the
			// part
			...
			// Remove any spaces and/or tabs from the end of the segment (would cause problems if
			// you left 'em).
	...
	/**
	 * A tab expander for the document currently being printed with the font being used for the
	 * printing.
	 */
