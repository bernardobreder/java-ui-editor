...
public abstract class AbstractMarkupLanguageSupport extends AbstractLanguageSupport {
	 ...
	 * closing tags, based on the doctype. For XML, this will always occur if this
	 ...
	 * types a (non-self-closing) start tag. This will only be done for tags where
	 * closing tags are accepted and valid, based on the doctype.
	 *
	...
	/**
	 * Installs extra keyboard shortcuts supported by this language support. The
	 * default implementation maps an action to automatically add closing tags when
	 * '&gt;' is pressed; subclasses can override and add additional shortcuts if
	 * desired.
	 * <p>
	 * Subclasses should call this method in their {@link #install(RSyntaxTextArea)}
	 * methods.
	 *
	...
	/**
	 * Subclasses should override this method to return whether a specified tag
	 * should have its closing tag auto-inserted.
	 *
	 ...
	 * (non-self-closing) start tag. This will only be done for tags where closing
	 * tags are accepted and valid, based on the doctype.
	 *
	...
	/**
	 * Action that checks whether a closing tag should be auto-inserted into the
	 * document. Subclasses should map this action to the '&gt;' key-typed event.
	 */
