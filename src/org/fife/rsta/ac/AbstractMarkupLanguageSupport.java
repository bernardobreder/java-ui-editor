/*
 * 01/29/2013 Copyright (C) 2013 Robert Futrell robert_futrell at users.sourceforge.net
...
/**
 * Base class for language supports for markup languages, such as HTML, PHP, and JSP. This class
 * facilitates support for automatically adding a closing tag (e.g. "<code>&lt;/foo&gt;</code>
 * ") when the user types an opening tag (e.g. " <code>&lt;foo attr='val'&gt;</code>").
 *
...
public abstract class AbstractMarkupLanguageSupport extends AbstractLanguageSupport {
	...
	/**
	 * Whether closing tags are automatically added when the user types opening tags. For HTML, this
	 * will only occur for tags that are allowed to have closing tags, based on the doctype. For
	 * XML, this will always occur if this property is set to <code>true</code>.
	 */
	...
	/**
	 * Returns whether closing tags should be automatically added when the user types a
	 * (non-self-closing) start tag. This will only be done for tags where closing tags are accepted
	 * and valid, based on the doctype.
	 *
	...
	/**
	 * Installs extra keyboard shortcuts supported by this language support. The default
	 * implementation maps an action to automatically add closing tags when '&gt;' is pressed;
	 * subclasses can override and add additional shortcuts if desired.
	 * <p>
	...
	/**
	 * Subclasses should override this method to return whether a specified tag should have its
	 * closing tag auto-inserted.
	 *
	...
	/**
	 * Sets whether closing tags should be automatically added when the user types a
	 * (non-self-closing) start tag. This will only be done for tags where closing tags are accepted
	 * and valid, based on the doctype.
	 *
	 ...
	 * Subclasses should map this action to the '&gt;' key-typed event.
	 */
	private class InsertClosingTagAction extends TextAction {
		...
		/**
		 * Discovers the name of the tag just opened, if any. Assumes standard SGML-style markup
		 * tags.
		 *
		 * @param doc The document to parse.
		 * @param dot The location of the caret. This should be right at a " <code>&gt;</code>"
		 *            character closing an HTML tag.
			...
			while (t != null && t.isPaintable()) {
				if (t.getType() == Token.MARKUP_TAG_DELIMITER) {
					if (t.isSingleChar('<')) {
						...
						if (t.getOffset() == dot) {
							if (candidate == null || shouldAutoCloseTag(candidate)) {
								return candidate;
							}
							return null;
						}
