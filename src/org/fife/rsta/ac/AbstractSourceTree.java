/*
 * 10/09/2011 Copyright (C) 2011 Robert Futrell robert_futrell at users.sourceforge.net
...
/**
 * A tree showing the structure of a source file being edited in an <code>RSyntaxTextArea</code>.
 * This can be used to display an "outline view" of the code, for example.
 * <p>
 * Concrete implementations typically specialize in displaying code structure for a single language,
 * and are registered to listen to code changes in an <code>RSyntaxTextArea</code> instance by
 * calling {@link #listenTo(RSyntaxTextArea)}. They should then listen to document changes and
 * adjust themselves to reflect the code structure of the current content as best as possible.
 * <p>
 * You should only add instances of {@link SourceTreeNode} or subclasses to this tree. You should
 * also provide a no-argument constructor if you wish to use your subclass in
 * {@link GoToMemberAction}.
 *
...
public abstract class AbstractSourceTree extends JTree {
	...
	/**
	 * Expands all nodes in the specified tree. Subclasses should implement this in a way logical
	 * for the language.
	 */
	public abstract void expandInitialNodes();
	
	/**
	 * An attempt to quickly expand all tree nodes. Only need to expand the "deepest" nodes, as they
	 * will auto-expand all parents to make sure they are visible.
	 *
	 ...
	 *
	 * @param pattern The prefix, as a wildcard expression. If this is <code>null</code>, all
	...
	/**
	 * Returns whether, when a source element is selected in this tree, the same source element
	 * should be selected in the editor.
	 *
	...
	/**
	 * Returns whether only "major" structural elements are shown in this source tree. An example of
	 * a "minor" element could be a local variable in a function or method.
	 *
	 ...
	 *            {@link LanguageSupportFactory}, and be editing the language we're interested in.
	 * @see #uninstall()
	 */
	public abstract void listenTo(RSyntaxTextArea textArea);
	
	/**
	 * Refreshes what children are visible in the tree. This should be called manually when updating
	 * a source tree with a new root, and is also called internally on filtering and sorting.
	 */
	public void refresh() {
	...
	public void selectFirstNodeMatchingFilter() {
		...
		while (en.hasMoreElements()) {
			...
			JLabel renderer = (JLabel) getCellRenderer().getTreeCellRendererComponent(this, stn, true, true, stn.isLeaf(), 0, true);
	...
	/**
	 * Sets whether, when a source element is selected in this tree, the same source element should
	 * be selected in the editor.
	 *
	 * @param gotoSelectedElement Whether to highlight the source element in the editor.
	...
	/**
	 * Toggles whether only "major" structural elements should be shown in this source tree. An
	 * example of a "minor" element could be a local variable in a function or method.
	 *
