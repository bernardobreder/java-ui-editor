...
public class HtmlCompletionProvider extends DefaultCompletionProvider {
	...
	/**
	 * Whether the text last grabbed via {@link #getAlreadyEnteredText(JTextComponent)} was an HTML
	 * tag name.
	 *
	 * @see #lastTagName
	 */
	...
	/**
	 * Returns the last tag name grabbed via {@link #getAlreadyEnteredText(JTextComponent)}. This
	 * value is only valid if {@link #isTagName} is <code>false</code>.
	 */
	...
	/**
	 * This nasty hack is just a hook for subclasses (e.g. <code>PhpCompletionProvider</code>) to be
	 * able to get at the <code>DefaultCompletionProvider</code> implementation.
	 *
	...
	/**
	 * Locates the name of the tag a given offset is in. This method assumes that the caller has
	 * already concluded that <code>offs</code> is in fact inside a tag, and that there is a little
	 * "text" just before it.
	 *
			...
			if (dot > 0) { // Must go back 1
				...
				try {
					...
					if (list != null) { // Always true?
						...
						else if (t.getType() != Token.MARKUP_TAG_ATTRIBUTE && t.getType() != Token.MARKUP_TAG_NAME) {
	...
	/**
	 * Returns the attributes that can be code-completed for the specified tag. Subclasses can
	 * override this method to handle more than the standard set of HTML 5 tags and their
	 * attributes.
	 *
	 * @param tagName The tag whose attributes are being code-completed.
	 * @return A list of attributes, or <code>null</code> if the tag is not recognized.
	 */
	...
	/**
	 * Returns the completions for the basic tag set. This method is here so subclasses can add to
	 * it if they provide additional tags (i.e. JSP).
	 *
	 ...
	 * @return The token before the offset, or <code>null</code> if the offset was the first offset
	 *         in the token list (or not in the token list at all, which would be an error).
	 */
	...
	/**
	 * Calls {@link #loadFromXML(String)} to load all standard HTML completions. Subclasses can
	 * override to also load additional standard tags (i.e. JSP's <code>jsp:*</code> tags).
	 */
	...
	/**
	 * Returns whether the given offset is inside a markup tag (and not in string content, such as
	 * an attribute value).
	 *
	 * @param textArea The text area being parsed.
	 * @param list The list of tokens for the current line (the line containing <code>offs</code>.
	...
	/**
	 * Returns whether this token's text is "<" or "</". It is assumed that whether this is a markup
	 * delimiter token is checked elsewhere.
	 *
