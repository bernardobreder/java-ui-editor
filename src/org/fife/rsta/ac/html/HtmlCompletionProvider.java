...
public class HtmlCompletionProvider extends DefaultCompletionProvider {
	 ...
	 * {@link #getAlreadyEnteredText(JTextComponent)}. This value is only valid if
	...
	/**
	 * Locates the name of the tag a given offset is in. This method assumes that
	 * the caller has already concluded that <code>offs</code> is in fact inside a
	 * tag, and that there is a little "text" just before it.
	 *
	 ...
	 * Subclasses can override this method to handle more than the standard set of
	 * HTML 5 tags and their attributes.
	 *
	 ...
	 * @return The token before the offset, or <code>null</code> if the offset was
	 *         the first offset in the token list (or not in the token list at all,
	...
	/**
	 * Returns whether the given offset is inside a markup tag (and not in string
	 * content, such as an attribute value).
	 *
	...
	/**
	 * Returns whether this token's text is "<" or "</". It is assumed that whether
	 * this is a markup delimiter token is checked elsewhere.
	 *
