...
/**
 * Manages a list of jars and gets completions from them. This can be shared amongst multiple
 * {@link JavaCompletionProvider} instances.
 *
...
public class JarManager {
	...
	public void addCompletions(CompletionProvider p, String text, Set<Completion> addTo) {
		/*
		 * The commented-out code below is probably replaced by the rest of the code in this
		 * method... TODO: Verify me!!! // Add any completions matching the text for each jar we
		 * know about String[] pkgNames = Util.splitOnChar(text, '.'); for (int i=0; i<jars.size();
		 * i++) { JarReader jar = (JarReader)jars.get(i); jar.addCompletions(p, pkgNames, addTo); }
		 */
		if (text.length() == 0) {
			return;
		}
	...
	/**
	 * Adds a jar to read from. This is a convenience method for folks only reading classes from jar
	 * files.
	 *
	 * @param jarFile The jar to add. This cannot be <code>null</code>.
	 * @return Whether this jar was added (e.g. it wasn't already loaded, or it has a new source
	 *         path).
	 ...
	 *
	 * @param info The source to add. If this is <code>null</code>, then the current JVM's main JRE
	 *            jar (rt.jar, or classes.jar on OS X) will be added. If this source has already
	 *            been added, adding it again will do nothing (except possibly update its attached
	 *            source location).
	 * @return Whether this source was added (e.g. it wasn't already loaded, or it has a new source
	 *         path).
	 * @throws IOException If an IO error occurs.
	...
	/**
	 * Adds the current JVM's rt.jar (or class.jar if on OS X) to the list of class file sources. If
	 * the application is running in a JDK, the associated source zip is also located and used.
	 *
	 * @throws IOException If an IO error occurs.
	 * @see #addClassFileSource(LibraryInfo)
	 */
	...
	/**
	 * Returns whether the "last modified" time stamp on jars and class directories should be
	 * checked whenever completions are requested, and if the jar/directory has been modified since
	 * the last time, reload any cached class file data. This allows for code completion to update
	 * whenever dependencies are rebuilt, but has the side effect of increased file I/O. By default
	 * this option is enabled; if you somehow find the file I/O to be a bottleneck (perhaps
	 * accessing jars over a slow NFS mount), you can disable this option.
	 *
	 * @return Whether jars/directories are checked for modification since the last access, and
	 *         clear any cached completion information if so.
	...
	/**
	 * Returns a list of all classes/interfaces/enums with a given (unqualified) name. There may be
	 * several, since the name is unqualified.
	 *
	 * @param name The unqualified name of a type declaration.
	 * @param importDeclarations The imports of the compilation unit, if any.
	 * @return A list of type declarations with the given name, or <code>null</code> if there are
	 *         none.
	 */
	 ...
	 *
	 * @return A list of {@link LibraryInfo}s. Modifying a <code>LibraryInfo</code> in this list
	 *         will have no effect on this completion provider; in order to do that, you must re-add
	 *         the jar via {@link #addClassFileSource(LibraryInfo)}. If there are no jars on the
	 *         "build path," this will be an empty list.
	 * @see #addClassFileSource(LibraryInfo)
	 */
	public List<LibraryInfo> getClassFileSources() {
		List<LibraryInfo> jarList = new ArrayList<LibraryInfo>(classFileSources.size());
	...
	/**
	 * Removes a jar from the "build path." This is a convenience method for folks only adding and
	 * removing jar sources.
	 *
	 * @param jar The jar to remove.
	 * @return Whether the jar was removed. This will be <code>false</code> if the jar was not on
	 *         the build path.
	 * @see #removeClassFileSource(LibraryInfo)
	 * @see #addClassFileSource(LibraryInfo)
	 * @see #getClassFileSources()
	 ...
	 * @return Whether source jar was removed. This will be <code>false</code> if the source was not
	 *         on the build path.
	 * @see #removeClassFileSource(File)
	 * @see #addClassFileSource(LibraryInfo)
	 * @see #getClassFileSources()
	...
	/**
	 * Sets whether the "last modified" time stamp on jars and class directories should be checked
	 * whenever completions are requested, and if the jar/directory has been modified since the last
	 * time, reload any cached class file data. This allows for code completion to update whenever
	 * dependencies are rebuilt, but has the side effect of increased file I/O. By default this
	 * option is enabled; if you somehow find the file I/O to be a bottleneck (perhaps accessing
	 * jars over a slow NFS mount), you can disable this option.
	 *
	 * @param check Whether to check if any jars/directories have been modified since the last
	 *            access, and clear any cached completion information if so.
