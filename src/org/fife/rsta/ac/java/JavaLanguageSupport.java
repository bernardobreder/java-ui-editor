...
public class JavaLanguageSupport extends AbstractLanguageSupport {
	 ...
	 * @return The completion provider. This will be <code>null</code> if the text
	...
	/**
	 * Returns the shared jar manager instance. NOTE: This method will be removed
	 * over time, as the Java support becomes more robust!
	 *
	 ...
	 *
	 * @param textArea The text area.
	 * @return The Java parser. This will be <code>null</code> if the text area does
	 *         not have this <tt>JavaLanguageSupport</tt> installed.
	 */
	 ...
	 * This is used internally when a class completion is inserted, and it needs an
	 * import statement added to the source.
	 */
	 ...
	 * Unlike many simpler language supports, <tt>JavaLanguageSupport</tt> cannot
	 * share any information amongst instances of <tt>RSyntaxTextArea</tt>.
	 */
	...
	private class JavaAutoCompletion extends AutoCompletion {
		...
		/**
		 * Overridden to allow for prepending to the replacement text. This allows us to
		 * insert fully qualified class names. instead of unqualified ones, if necessary
		 * (i.e. if the user tries to auto-complete
		 * <code>javax.swing.text.Document</code>, but they've explicitly imported
		 * <code>org.w3c.dom.Document</code> - we need to insert the fully qualified
		 * name in that case).
		 */
		@Override
		...
		/**
		 * Determines whether the class name being completed has been imported, and if
		 * it hasn't, returns the import statement that should be added for it.
		 * Alternatively, if the class hasn't been imported, but a class with the same
		 * (unqualified) name HAS been imported, this method sets things up so the
		 * fully-qualified version of this class's name is inserted.
		 * <p>
		 * Thanks to Guilherme Joao Frantz and Jonatas Schuler for helping with the
		 * patch!
		 *
		...
		/**
		 * Overridden to handle special cases, because sometimes Java code completions
		 * will edit more in the source file than just the text at the current caret
		 * position.
		 */
		@Override
	...
	/**
	 * Listens for various events in a text area editing Java (in particular, caret
	 * events, so we can track the "active" code block).
	 */
