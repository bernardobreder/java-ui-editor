 ...
 * <li>Field names
 * </ul>
 * Also, if the caret is inside a method, local variables up to the caret position are also
 * returned.
 *
...
class SourceCompletionProvider extends DefaultCompletionProvider {
	...
	/**
	 * Adds completions for accessible methods and fields of super classes. This is only called when
	 * the caret is inside of a class. TODO: Handle accessibility correctly!
	 *
	 * @param set
	 * @param cu The compilation unit.
	 * @param cf A class in the chain of classes that a type being parsed inherits from.
	 * @param pkg The package of the source being parsed.
	 * @param typeParamMap A mapping of type parameters to type arguments for the object whose
	 *            fields/methods/etc. are currently being code-completed.
	 */
	...
	/**
	 * Adds completions for all methods and public fields of a local variable. This will add nothing
	 * if the local variable is a primitive type.
	 *
	 ...
	 * @return The {@link ClassFile} for the class, or <code>null</code> if <code>cf</code>
	 *         represents <code>java.lang.Object</code> (or if the super class could not be
	 *         determined).
	 */
	 ...
	 * @param set
	 * @param method
	 * @param offs The caret's offset into the source. This should be inside of <code>method</code>.
	 */
	 ...
	 * @param offs The caret's offset into the source. This should be inside of <code>block</code>.
	 */
	 ...
	 *
	 * @param info The jar to add. If this is <code>null</code>, then the current JVM's main JRE jar
	 *            (rt.jar, or classes.jar on OS X) will be added. If this jar has already been
	 *            added, adding it again will do nothing (except possibly update its attached source
	 *            location).
	 ...
	 *
	 * @param type The type of a variable/field/etc. whose fields/methods/etc. are being code
	 *            completed, as declared in the source. This includes type arguments.
	 * @param cf The <code>ClassFile</code> representing the actual type of the variable/field/etc.
	 *            being code completed
	 ...
	 *         the jar via {@link #addJar(LibraryInfo)}. If there are no jars on the "build path,"
	 ...
	 * <ul>
	 * <li>If the caret is anywhere in a class, the names of all methods and fields in the class are
	 * loaded. Methods and fields in super classes are also loaded. TODO: Get super methods/fields
	 * added correctly by access!
	 ...
	 * when the caret is found to be in a specific type declaration. This method checks if the caret
	 * is in a child type declaration first, then adds completions for itself next.
	 * <ul>
	 * <li>If the caret is anywhere in a class, the names of all methods and fields in the class are
	 * loaded. Methods and fields in super classes are also loaded. TODO: Get super methods/fields
	 * added correctly by access!
	...
	/**
	 * Loads completions for the text at the current caret position, if there is a "prefix" of chars
	 * and at least one '.' character in the text up to the caret. This is currently very limited
	 ...
	 * @param currentMethod The method the caret is in, or <code>null</code> if none.
	 * @param prefix The text up to the current caret position. This is guaranteed to be non-
	 *            <code>null</code> not equal to "<tt>this</tt>".
