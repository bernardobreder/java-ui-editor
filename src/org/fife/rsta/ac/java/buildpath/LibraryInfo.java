/*
 * 04/21/2012 Copyright (C) 2010 Robert Futrell robert_futrell at users.sourceforge.net
...
/**
 * Information about a jar, compiled class folder, or other source of classes to add to the
 * "build path" for Java completion. Instances of this class are added to a {@link JarManager} for
 * each library that should be on the build path.
 * <p>
 * This class also keeps track of an optional source location, such as a zip file or source folder.
 * If defined, this location is used to find the .java source corresponding to the library's
 * classes, which is used to display Javadoc comments during code completion.
 *
...
public abstract class LibraryInfo implements Comparable, Cloneable {
	 ...
	 * {@link #createClassFileBulk(String)}. After calling this method, the actual class file
	 * fetching should be done in a try/finally block that ensures a call to
	 * {@link #bulkClassFileCreationEnd()}; e.g.
	 *
	 * <pre>
	 ...
	 * <code>JarReader</code> can call this method to lazily load information on individual classes
	 * and shove it into their package maps.
	 * <p>
	 * If many class files will be fetched at a time, you should prefer using
	 * {@link #bulkClassFileCreationStart()} and {@link #createClassFileBulk(String)} over this
	 ...
	 * and shove it into their package maps.
	 * <p>
	 * This method should be used when multiple classes will be fetched from this library at the
	 * same time. It should only be called after a call to {@link #bulkClassFileCreationStart()}. If
	 * only a single class file is being fetched, it is simpler to call
	...
	/**
	 * Creates and returns a map of maps representing the hierarchical package structure in this
	 * library.
	 *
	...
	/**
	 * Two <code>LibraryInfo</code>s are considered equal if they represent the same class file
	...
	/**
	 * Returns information on the "main" jar for a JRE. This will be <tt>rt.jar</tt> everywhere
	 * except OS X, where it will be <tt>classes.jar</tt>. The associated source zip/jar file is
	 * also checked for.
	 *
	 * @return The information, or <code>null</code> if there is not a JRE in the specified
	 *         directory.
	...
	/**
	 * Returns the time this library was last modified. For jar files, this would be the modified
	 * date of the file. For directories, this would be the time a file in the directory was most
	 * recently modified. This information is used to determine whether callers should clear their
	 * cached package map information and load it anew.
	 * <p>
	...
	/**
	 * Returns the location of this library, as a string. If this library is contained in a single
	 * jar file, this will be the full path to that jar. If it is a directory containing classes, it
	 * will be the full path of the directory. Otherwise, this value will be <code>null</code>.
	 *
	...
	/**
	 * Returns information on the JRE running this application. This will be <tt>rt.jar</tt>
	 * everywhere except OS X, where it will be <tt>classes.jar</tt>. The associated source zip/jar
	 * file is also checked for.
	 *
	...
	/**
	 * Subclasses should override this method since {@link #equals(Object)} is overridden. Instances
	 * of <code>LibraryInfo</code> aren't typically stored in maps, so the hash value isn't
	 * necessarily important to <code>RSTALanguageSupport</code>.
	 *
