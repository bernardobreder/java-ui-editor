...
public class ASTFactory implements TokenTypes {
	...
	private CodeBlock _getBlock(CompilationUnit cu, CodeBlock parent, Method m, Scanner s, boolean isStatic, int depth)
		...
		OUTER: while (true) {
			...
			case KEYWORD_TRY:
				...
				block.add(tryBlock);
				while (s.yyPeekCheckType() == KEYWORD_CATCH && s.yyPeekCheckType(2) == SEPARATOR_LPAREN) {
					s.yylex(); // catch
					s.yylex(); // lparen
					Type exType = null;
					Token var = null;
		...
		try {
			...
			OUTER: while (t != null && t.isType(KEYWORD_IMPORT)) {
				...
				if (!t.isIdentifier()) {
					...
					temp = scanner.yylexNonNull(SEPARATOR_DOT, SEPARATOR_SEMICOLON, "'.' or ';' expected");
					while (temp.isType(SEPARATOR_DOT)) {
						temp = scanner.yylexNonNull(IDENTIFIER, OPERATOR_TIMES, "Identifier or '*' expected");
	...
	private void _getInterfaceMemberDecl(CompilationUnit cu, Scanner s, NormalInterfaceDeclaration iDec,
		...
		if (varDecl) {
			...
			Type type = null;
			if (methodNameAndTypeTokenList.size() > 1) { // InterfaceMethodOrFieldDecl or
	...
	private void _getMemberDecl(CompilationUnit cu, Scanner s, NormalClassDeclaration classDec, Modifiers modList)
		...
		if (varDecl) {
			...
			Scanner tempScanner = new Scanner(methodNameAndTypeTokenList);
			Type type = null;
			if (methodNameAndTypeTokenList.size() > 1) { // GenericMethodOrConstructorDecl, or
															// method (not a regular constructor).
	...
	private TypeArgument _getTypeArgument(CompilationUnit cu, Scanner s) throws IOException {
		...
		if (t.isType(OPERATOR_QUESTION)) {
			...
			if (t.getType() != OPERATOR_GT) {
				t = s.yylexNonNull(SEPARATOR_COMMA, KEYWORD_EXTENDS, KEYWORD_SUPER, "',', super or extends expected");
				switch (t.getType()) {
