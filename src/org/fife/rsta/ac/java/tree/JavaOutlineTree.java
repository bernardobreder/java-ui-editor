 ...
 * It also uses Eclipse's icons, just like the rest of this code completion library.
 * <p>
 * You can get this tree automatically updating in response to edits in an
 * <code>RSyntaxTextArea</code> with {@link JavaLanguageSupport} installed by calling
 ...
 * you'll either need a separate <code>JavaOutlineTree</code> for each one, or call
...
public class JavaOutlineTree extends AbstractSourceTree {
	 ...
	 * Refreshes this tree.
	 *
	 * @param cu The parsed compilation unit. If this is <code>null</code> then the tree is cleared.
	 */
	...
	private void checkForJavaParsing() {
		...
		if (parser != null) { // Should always be true
			parser.addPropertyChangeListener(JavaParser.PROPERTY_COMPILATION_UNIT, listener);
