/*
 * 07/05/2011 Copyright (C) 2011 Robert Futrell robert_futrell at users.sourceforge.net
...
public class JspCompletionProvider extends HtmlCompletionProvider {
	...
	public JspCompletionProvider() {
		 ...
		 * "c:/dev/struts/struts-2.2.3/lib/struts2-core-2.2.3.jar"; File file = new
		 * File(fileName); if (!file.exists()) { file = new
		 * File("C:/temp/struts2-core-2.2.1.jar"); } try { prefixToTld.put("s", new
		 * TldFile(this, file)); } catch (IOException ioe) { ioe.printStackTrace(); }
		 */
	...
	protected List<Completion> getTagCompletions() {
		...
		List<Completion> completions = new ArrayList<Completion>(super.getTagCompletions());
		...
		for (Map.Entry<String, TldFile> entry : prefixToTld.entrySet()) {
			...
			for (int j = 0; j < tld.getElementCount(); j++) {
				...
				MarkupTagCompletion mtc = new MarkupTagCompletion(this, prefix + ":" + elem.getName());
