 ...
 * current caret position are suggested, but there may still be issues with
 * variable types, etc.</li>
 * </ul>
...
public class PerlCompletionProvider extends CCompletionProvider {
	...
	/**
	 * Creates an "AST" for Perl code, representing code blocks and variables inside
	 * of those blocks.
	 *
	...
	/**
	 * Overridden to return the null char (meaning "no end character") if the user
	 * doesn't want to use parens around their functions.
	 *
	...
	/**
	 * Overridden to return the null char (meaning "no start character") if the user
	 * doesn't want to use parens around their functions.
	 *
	...
	/**
	 * Does a crude search for variables up to the caret position. This method does
	 * not care whether the variables are in scope at the caret position.
	 *
	...
	/**
	 * Recursively adds any local variables defined before the given caret offset,
	 * and in the given code block (and any nested children the caret is in).
	 *
