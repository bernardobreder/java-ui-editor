 ...
 * <ul>
 * <li>Auto-completion for standard Perl 5.10 functions (read from an XML file).</li>
 * <li>Crude auto-completion for variables. Only variables in scope at the current caret position
 * are suggested, but there may still be issues with variable types, etc.</li>
 * </ul>
 * To toggle whether parameter assistance wraps your parameters in parens, use the
 * {@link #setUseParensWithFunctions(boolean)} method.
 *
...
public class PerlCompletionProvider extends CCompletionProvider {
	...
	/**
	 * Creates an "AST" for Perl code, representing code blocks and variables inside of those
	 * blocks.
	 *
	...
	/**
	 * Overridden to return the null char (meaning "no end character") if the user doesn't want to
	 * use parens around their functions.
	 *
	...
	/**
	 * Overridden to return the null char (meaning "no start character") if the user doesn't want to
	 * use parens around their functions.
	 *
	 * @return The start character for parameters list, or the null char if none.
	 * @see #getUseParensWithFunctions()
	 */
	@Override
	...
	/**
	 * Does a crude search for variables up to the caret position. This method does not care whether
	 * the variables are in scope at the caret position.
	 *
	 * @param comp The text area.
	 * @return The completions for variables, or <code>null</code> if there were none.
	 */
	...
	/**
	 * A comparator that compares the input text of two {@link Completion}s lexicographically,
	 * ignoring case.
	 */
	private static class CaseInsensitiveComparator implements Comparator<Completion>, Serializable {
	...
	/**
	 * Recursively adds any local variables defined before the given caret offset, and in the given
	 * code block (and any nested children the caret is in).
	 *
