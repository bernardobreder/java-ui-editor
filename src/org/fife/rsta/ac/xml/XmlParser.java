...
/**
 * Parses XML code in an <code>RSyntaxTextArea</code>.
 * <p>
 * Like all RSTA <code>Parser</code>s, an <code>XmlParser</code> instance is notified when the
 * RSTA's text content changes. After a small delay, it will parse the content as XML, building an
 * AST and looking for any errors. When parsing is complete, a property change event of type
 * {@link #PROPERTY_AST} is fired. Listeners can check the new value of the property for an
 * {@link XmlTreeNode} that represents the root of a tree structure modeling the XML content in the
 * text area. Note that the <code>XmlTreeNode</code> may be incomplete if there were parsing/syntax
 * errors (it will usually be complete "up to" the error in the content).
 * <p>
 * This parser cannot be shared amongst multiple instances of <code>RSyntaxTextArea</code>.
 * <p>
 *
...
public class XmlParser extends AbstractParser {
	 ...
	 * listen for this property change and update themselves accordingly. The property's "new value"
	 * will be an {@link XmlTreeNode} representing the root of a tree modeling the XML content. The
	 * "old value" is always <code>null</code>.
	 */
	...
	/**
	 * Adds a listener to this parser. Typically you'd want to register a listener on the
	 * {@link #PROPERTY_AST} property.
	 *
	 ...
	 *
	 * @return The root node of the XML model, or <code>null</code> if it has not yet been parsed or
	 ...
	 *
	 * @param attributes The attributes of an element. Calling code should have already verified
	 *            this has length &gt; 0.
	...
	/**
	 * Sets how validation will be done by this parser. This can be used to set up either DTD or
	 * Schema validation.
	 *
	 * @param config Configuration information for validation. If this is <code>null</code>, no
	...
	/**
	 * Callback for events when we're parsing the XML in the editor. Creates our model and records
	 * any parsing errors for squiggle underlining.
	 */
	public class Handler extends DefaultHandler {
		...
		public void startElement(String uri, String localName, String qName, Attributes attributes) {
			...
			if (locator != null) {
				...
				if (line != -1) {
					int offs = doc.getDefaultRootElement().getElement(line - 1).getStartOffset();
