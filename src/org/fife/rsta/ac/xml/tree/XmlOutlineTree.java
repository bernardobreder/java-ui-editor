/*
 * 04/07/2012 Copyright (C) 2012 Robert Futrell robert_futrell at users.sourceforge.net
...
/**
 * A tree view showing the outline of XML, similar to the "Outline" view in Eclipse. It also uses
 * Eclipse's icons, just like the rest of this code completion library.
 * <p>
 * You can get this tree automatically updating in response to edits in an
 * <code>RSyntaxTextArea</code> with {@link XmlLanguageSupport} installed by calling
 * {@link #listenTo(RSyntaxTextArea)}. Note that an instance of this class can only listen to a
 * single editor at a time, so if your application contains multiple instances of RSyntaxTextArea,
 * you'll either need a separate <code>XmlOutlineTree</code> for each one, or call
 * <code>uninstall()</code> and <code>listenTo(RSyntaxTextArea)</code> each time a new RSTA receives
 * focus.
 *
...
public class XmlOutlineTree extends AbstractSourceTree {
	 ...
	 * Constructor.
	 *
	 * @param sorted Whether the tree should sort its elements alphabetically. Note that outline
	 *            trees will likely group nodes by type before sorting (i.e. methods will be sorted
	 *            in one group, fields in another group, etc.).
	 */
	...
	private void checkForXmlParsing() {
		...
		LanguageSupport support = lsf.getSupportFor(SyntaxConstants.SYNTAX_STYLE_XML);
	...
	/**
	 * Listens for events this tree is interested in (events in the associated editor, for example),
	 * as well as events in this tree.
	 */
	private class XmlEditorListener implements PropertyChangeListener, TreeSelectionListener {
		...
		/**
		 * Selects the corresponding element in the text editor when a user clicks on a node in this
		 * tree.
		 */
		@Override
	...
	/**
	 * Whenever the caret moves in the listened-to RSyntaxTextArea, this class ensures that the XML
	 * element containing the caret position is focused in the tree view after a small delay.
	 */
	/*
	 * TODO: Make me work for any LanguageSupport (don't synchronize if waiting on a pending parse)
	 * and pull me out and make me available for all languages. private class Synchronizer
	 * implements CaretListener, ActionListener { private Timer timer; private int dot; public
	 * Synchronizer() { timer = new Timer(650, this); timer.setRepeats(false); } public void
	 * actionPerformed(ActionEvent e) { recursivelyCheck(root); //System.out.println("Here"); }
	 * public void caretUpdate(CaretEvent e) { this.dot = e.getDot(); timer.restart(); } private
	 * boolean recursivelyCheck(XmlTreeNode node) { if (node.containsOffset(dot)) { for (int i=0;
	 * i<node.getChildCount(); i++) { XmlTreeNode child = (XmlTreeNode)node.getChildAt(i); if
	 * (recursivelyCheck(child)) { return true; } } // None of the children contain the offset, must
	 * this guy node.selectInTree(); return true; } return false; } }
	 */
