/*
 * 01/15/2004 RComboBoxModel.java - A combo box model that limits the number of items the combo box
 * wil remember. It also won't add an item to the combo box if it is already there. This library is
...
/**
 * A combo box model that limits the number of items the combo box will "remember." You can use it
 * like so: <br>
 ...
 * <br>
 * It also won't let you add an item to the combo box twice (i.e., no duplicates), and it adds new
 * items to the beginning of the list, not the end (as <code>JComboBox</code>'s do by default).<br>
 * <br>
...
public class RComboBoxModel extends DefaultComboBoxModel {
	...
	/**
	 * Adds the object (if it's not already in the list) to the front of the list. If it's already
	...
	/**
	 * Adds an item at a specified index. The implementation of this method should notify all
	 * registered <code>ListDataListeners</code> that the item has been added.
	 *
	 ...
	 *
	 * @param numElements The maximum number of items this combo box can hold. If
	 *            <code>numElements <= 0</code>, then the capacity of this combo box is set to
	 *            <code>4</code>.
	 */
