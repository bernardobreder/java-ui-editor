/*
 * 11/14/2003 ReplaceDialog.java - Dialog for replacing text in a GUI. This library is distributed
 * under a modified BSD license. See the included RSTAUI.License.txt file for details.
 */
...
/**
 * A "Replace" dialog similar to those found in most Windows text editing applications. Contains
 * many search options, including:<br>
 * <ul>
 * <li>Match Case
 * <li>Match Whole Word
 ...
 * <p>
 * An application can use a <code>ReplaceDialog</code> as follows. It is suggested that you create
 * an <code>Action</code> or something similar to facilitate "bringing up" the Replace dialog. Have
 * the main application contain an object that implements <code>ActionListener</code>. This object
 * will receive the following action events from the Replace dialog:
 * <ul>
 * <li>{@link SearchEvent.Type#FIND} action when the user clicks the "Find" button.
 * <li>{@link SearchEvent.Type#REPLACE} action when the user clicks the "Replace" button.
 * <li>{@link SearchEvent.Type#REPLACE_ALL} action when the user clicks the "Replace All" button.
 * </ul>
...
public class ReplaceDialog extends AbstractFindReplaceDialog {
	 ...
	 *
	 * @return The <code>String</code> the user wants to replace the text to find with.
	 */
	...
	/**
	 * Called when the regex checkbox is clicked. Subclasses can override to add custom behavior,
	 * but should call the super implementation.
	 */
	@Override
	 ...
	 *
	 * @param newReplaceString The <code>String</code> to put into the replace field.
	 */
	...
	/**
	 * Overrides <code>JDialog</code>'s <code>setVisible</code> method; decides whether or not
	 * buttons are enabled.
	 *
	...
	/**
	 * This method should be called whenever the <code>LookAndFeel</code> of the application
	 * changes. This calls <code>SwingUtilities.updateComponentTreeUI(this)</code> and does other
	 * necessary things.
	 * <p>
	 * Note that this is <em>not</em> an override, as JDialogs don't have an <code>updateUI()</code>
	 * method.
	 */
