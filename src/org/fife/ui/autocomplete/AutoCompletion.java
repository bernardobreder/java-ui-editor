/*
 * 12/21/2008 AutoCompletion.java - Handles auto-completion for a text component. This library is
...
/**
 * Adds auto-completion to a text component. Provides a popup window with a list of auto-complete
 * choices on a given keystroke, such as Crtrl+Space.
 * <p>
 * Depending on the {@link CompletionProvider} installed, the following auto-completion features may
 * be enabled:
 * <ul>
 * <li>An auto-complete choices list made visible via e.g. Ctrl+Space</li>
 * <li>A "description" window displayed alongside the choices list that provides documentation on
 * the currently selected completion choice (as seen in Eclipse and NetBeans).</li>
 * <li>Parameter assistance. If this is enabled, if the user enters a "parameterized" completion,
 * such as a method or a function, then they will receive a tool tip describing the arguments they
 * have to enter to the completion. Also, the arguments can be navigated via tab and shift+tab (a la
 ...
 */
/*
 * This class handles intercepting window and hierarchy events from the text component, so the popup
 * window is only visible when it should be visible. It also handles communication between the
 * CompletionProvider and the actual popup Window.
 */
public class AutoCompletion {
	...
	/**
	 * The preferred size of the completion choices window. This field exists because the user will
	 * likely set the preferred size of the window before it is actually created.
	 */
	private Dimension preferredChoicesWindowSize;
	
	/**
	 * The preferred size of the optional description window. This field only exists because the
	 * user may (and usually will) set the size of the description window before it exists (it must
	 * be parented to a Window).
	 */
	...
	/**
	 * The renderer to use for the completion choices. If this is <code>null</code>, then a default
	 * renderer is used.
	 */
	...
	/**
	 * Whether the auto-activation of auto-complete (after a delay, after the user types an
	 * appropriate character) is enabled.
	 */
	private boolean autoActivationEnabled;
	
	/**
	 * Whether or not, when there is only a single auto-complete option that matches the text at the
	 * current text position, that text should be auto-inserted, instead of the completion window
	 * displaying.
	 */
	...
	/**
	 * A renderer used for {@link Completion}s in the optional parameter choices popup window
	 * (displayed when a {@link ParameterizedCompletion} is code-completed). If this isn't set, a
	...
	/**
	 * The action previously assigned to {@link #trigger}, so we can reset it if the user disables
	 * auto-completion.
	 */
	private Action oldTriggerAction;
	
	/**
	 * The previous key in the text component's <code>InputMap</code> for the parameter completion
	 * trigger key.
	 */
	private Object oldParenKey;
	
	/**
	 * The action previously assigned to the parameter completion key, so we can reset it when we
	 * uninstall.
	 */
	...
	/**
	 * Listens for events in the text component that cause the popup windows to automatically
	 * activate.
	 */
	private AutoActivationListener autoActivationListener;
	
	/**
	 * Listens for LAF changes so the auto-complete windows automatically update themselves
	 * accordingly.
	 */
	...
	/**
	 * Whether or not the popup should be hidden when user types a space (or any character that
	 * resets the completion list to "all completions"). Defaults to true.
	 */
	private boolean hideOnNoText;
	
	/**
	 * Whether or not the popup should be hidden when the CompletionProvider changes. If set to
	 * false, caller has to ensure refresh of the popup content. Defaults to true.
	 */
	...
	/**
	 * Displays the popup window. Hosting applications can call this method to programmatically
	 * begin an auto-completion operation.
	 */
	...
	/**
	 * Returns the delay between when the user types a character and when the code completion popup
	 * should automatically appear (if applicable).
	 *
	...
	/**
	 * Returns whether, if a single auto-complete choice is available, it should be automatically
	 * inserted, without displaying the popup menu.
	 *
	...
	/**
	 * Returns the default auto-complete "trigger key" for this OS. For Windows, for example, it is
	 * Ctrl+Space.
	 *
	...
	/**
	 * Returns the default list cell renderer used when a completion provider does not supply its
	 * own.
	 *
	...
	/**
	 * Returns the renderer to use for {@link Completion}s in the optional parameter choices popup
	 * window (displayed when a {@link ParameterizedCompletion} is code-completed). If this returns
	...
	/**
	 * Returns the text to replace with in the document. This is a "last-chance" hook for subclasses
	 * to make special modifications to the completion text inserted. The default implementation
	 * simply returns <tt>c.getReplacementText()</tt>. You usually will not need to modify this
	 * method.
	 *
	...
	/**
	 * Returns the style context describing how auto-completion related highlights in the editor are
	 * rendered.
	 *
	 ...
	 *
	 * @return The text component, or <code>null</code> if this {@link AutoCompletion} is not
	 *         installed on any text component.
	 ...
	 *
	 * @return The orientation of the text component, or <code>null</code> if we are not installed
	 *         on one.
	 */
	...
	/**
	 * Determines whether debug should be enabled for the AutoCompletion library. This method checks
	 * a system property, but takes care of {@link SecurityException}s in case we're in an applet or
	 * WebStart.
	 *
	...
	/**
	 * Inserts a completion. Any time a code completion event occurs, the actual text insertion
	 * happens through this method.
	 *
	 ...
	 *
	 * @param c A completion to insert. This cannot be <code>null</code>.
	 * @param typedParamListStartChar Whether the parameterized completion start character was typed
	 *            (typically <code>'('</code>).
	 */
	...
	/**
	 * Installs this auto-completion on a text component. If this {@link AutoCompletion} is already
	 * installed on another text component, it is uninstalled first.
	 *
	...
	/**
	 * Creates and returns the action to call when the user presses the auto-completion trigger key
	 * (e.g. ctrl+space). This is a hook for subclasses that want to provide their own behavior in
	 * this scenario. The default implementation returns an {@link AutoCompleteAction}.
	 *
	...
	/**
	 * Returns whether auto-activation is enabled (that is, whether the completion popup will
	 * automatically appear after a delay when the user types an appropriate character). Note that
	 * this parameter will be ignored if auto-completion is disabled.
	 *
	...
	/**
	 * Whether or not the popup should be hidden when the CompletionProvider changes. If set to
	 * false, caller has to ensure refresh of the popup content.
	 *
	...
	/**
	 * Whether or not the popup should be hidden when user types a space (or any character that
	 * resets the completion list to "all completions").
	 *
	...
	/**
	 * Refreshes the popup window. First, this method gets the possible completions for the current
	 * caret position. If there are none, and the popup is visible, it is hidden. If there are some
	 * completions and the popup is hidden, it is made visible and made to display the completions.
	 * If there are some completions and the popup is visible, its list is updated to the current
	 * set of completions.
	 *
	...
	/**
	 * Sets the delay between when the user types a character and when the code completion popup
	 * should automatically appear (if applicable).
	 *
	...
	/**
	 * Toggles whether auto-activation is enabled. Note that auto-activation also depends on
	 * auto-completion itself being enabled.
	 *
	...
	/**
	 * Sets whether, if a single auto-complete choice is available, it should be automatically
	 * inserted, without displaying the popup menu.
	 *
	...
	/**
	 * Sets the handler to use when an external URL is clicked in the description window. This
	 * handler can perform some action, such as open the URL in a web browser. The default
	 * implementation will open the URL in a browser, but only if running in Java 6. If you want
	 * browser support for Java 5 and below, or otherwise want to respond to hyperlink clicks, you
	...
	/**
	 * Sets whether or not the popup should be hidden when the CompletionProvider changes. If set to
	 * false, caller has to ensure refresh of the popup content.
	 *
	...
	/**
	 * Sets whether or not the popup should be hidden when user types a space (or any character that
	 * resets the completion list to "all completions").
	 *
	 * @see #isHideOnNoText()
	 */
	...
	/**
	 * Sets the redirector for external URL's found in code completion documentation. When a
	 * non-local link in completion popups is clicked, this redirector is given the chance to modify
	 * the URL fetched and displayed.
	 *
	...
	/**
	 * Sets the default list cell renderer to use when a completion provider does not supply its
	 * own.
	 *
	 * @param renderer The renderer to use. If this is <code>null</code>, a default renderer is
	 *            used.
	...
	/**
	 * Sets the renderer to use for {@link Completion}s in the optional parameter choices popup
	 * window (displayed when a {@link ParameterizedCompletion} is code-completed). If this isn't
	 * set, a default renderer is used.
	 *
	...
	/**
	 * Sets whether parameter assistance is enabled. If parameter assistance is enabled, and a
	 * "parameterized" completion (such as a function or method) is inserted, the user will get
	 * "assistance" in inserting the parameters in the form of a popup window with documentation and
	 * easy tabbing through the arguments (as seen in Eclipse and NetBeans).
	 *
	 ...
	 * @param typedParamListStartChar Whether the parameterized completion list starting character
	 *            was typed.
	 */
	...
	/**
	 * Uninstalls this auto-completion from its text component. If it is not installed on any text
	...
	/**
	 * Replaces the "trigger key" action with the one that was there before auto-completion was
	 * installed.
	 *
	...
	/**
	 * Updates the LookAndFeel of the popup window. Applications can call this method as appropriate
	 * if they support changing the LookAndFeel at runtime.
	 */
	...
	/**
	 * The <code>Action</code> that displays the popup window if auto-completion is enabled.
	 */
	...
	/**
	 * Listens for LookAndFeel changes and updates the various popup windows involved in
	 * auto-completion accordingly.
	 */
	...
	private class TextComponentListener extends FocusAdapter implements HierarchyListener {
		...
		/**
		 * Called when the component hierarchy for our text component changes. When the text
		 * component is added to a new {@link Window}, this method registers listeners on that
		 * <code>Window</code>.
		 *
		 * @param e The event.
		 */
