/*
 * 12/23/2008 CompletionCellRenderer.java - Cell renderer that can render the standard completion
 * types like Eclipse or NetBeans does. This library is distributed under a modified BSD license.
...
/**
 * A cell renderer that adds some pizazz when rendering the standard {@link Completion} types, like
 * Eclipse and NetBeans do. Specifically, this renderer handles:
 * <ul>
...
public class CompletionCellRenderer extends DefaultListCellRenderer {
	...
	/**
	 * The alternating background color, or <code>null</code> if alternating row colors should not
	 * be used.
	 */
	private static Color altBG;
	
	/**
	 * The font to use when rendering items, or <code>null</code> if the list's default font should
	 * be used.
	 */
	private Font font;
	...
	/**
	 * During rendering, this is the "real" background color of the item being rendered (i.e., what
	 * its background color is if it isn't selected).
	 */
	...
	/**
	 * Constructor. This is primarily a hook for Substance, or any other Look and Feel whose
	 * renderers look drastically different than standard <code>DefaultListCellRenderer</code>s.
	 * Everything except for the text rendering will be done by the delegate. In almost all
	 * scenarios, you will want to use the no-argument constructor instead of this one.
	 *
	...
	/**
	 * Creates the icon to use if no icon is found for a specific completion. The default
	 * implementation returns a 16x16 empty icon.
	 *
	 * @return The icon.
	 * @see #getEmptyIcon()
	 */
	...
	/**
	 * Attempts to delegate rendering to a Substance cell renderer. This should only be called if
	 * Substance is known to be on the classpath.
	 *
	 * @throws Exception If Substance is not on the classpath, or some other error occurs creating
	 *             the Substance cell renderer.
	 ...
	 *
	 * @return The alternate background color. If this is <code>null</code>, alternating colors are
	 *         not used.
	 ...
	 *
	 * @return The font. If this is <code>null</code>, then the default list font is used.
	...
	/**
	 * Returns the icon to use if no icon is found for a specific completion. This icon is lazily
	 ...
	 * Returns an icon.
	 *
	 * @param resource The icon to retrieve. This should either be a file, or a resource loadable by
	...
	/**
	 * Returns whether the types of fields and return types of methods are shown in the completion
	 * text.
	 *
	 ...
	 *
	 * @param altBG The new alternate background color. If this is <code>null</code>, alternating
	 *            lines will not use different background colors.
	...
	/**
	 * Sets the delegate renderer. Most users will never use this method; it is primarily a hook for
	 * Substance and other Look and Feels whose renderers look drastically different from the
	 * standard <code>DefaultListCellRenderer</code>.
	 *
	 * @param delegate The new delegate renderer. If this is <code>null</code>, the default
	 *            rendering of this component is used.
	 ...
	 *
	 * @param font The font to use. If this is <code>null</code>, then the default list font is
	 *            used.
	 * @see #getDisplayFont()
	 */
	...
	/**
	 * Sets the icon to display based off of a completion, falling back to the empty icon if the
	 * completion has no icon.
	 *
	...
	/**
	 * Sets the icon to display based off of a completion, falling back to a default icon if the
	 * completion has no icon.
	 *
	 * @param completion The completion to check.
	 * @param defaultIcon The icon to use if <code>completion</code> does not specify an icon.
	...
	/**
	 * Sets whether the types of fields and return types of methods are shown in the completion
	 * text.
	 *
	...
	/**
	 * Sets the color to use for function/field types. Note that if {@link #getShowTypes()} returns
	 * <code>false</code>, this property effectively does nothing.
	 *
	 * @param color The color to use for types. This is ignored if <code>null</code>.
	 * @see #setShowTypes(boolean)
