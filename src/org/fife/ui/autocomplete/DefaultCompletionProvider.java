/*
 * 12/21/2008 DefaultCompletionProvider.java - A basic completion provider implementation. This
...
/**
 * A basic completion provider implementation. This provider has no understanding of language
 * semantics. It simply checks the text entered up to the caret position for a match against known
 * completions. This is all that is needed in the majority of cases.
 *
...
public class DefaultCompletionProvider extends AbstractCompletionProvider {
	...
	/**
	 * Used to speed up {@link #getCompletionsAt(JTextComponent, Point)}, since this may be called
	 * multiple times in succession (this is usually called by
	 * <tt>JTextComponent.getToolTipText()</tt>, and if the user wiggles the mouse while a tool tip
	 * is displayed, this method gets repeatedly called. It can be costly so we try to speed it up a
	 * tad).
	 */
	 ...
	 *
	 * @param words The words to offer as completion suggestions. If this is <code>null</code>, no
	 *            completions will be known.
	 * @see BasicCompletion
	 */
	 ...
	 * {@link #isValidChar(char)}. {@inheritDoc}
	 */
	@Override
	...
	/**
	 * Returns whether the specified character is valid in an auto-completion. The default
	 * implementation is equivalent to " <code>Character.isLetterOrDigit(ch) || ch=='_'</code>".
	 * Subclasses can override this method to change what characters are matched.
	 *
