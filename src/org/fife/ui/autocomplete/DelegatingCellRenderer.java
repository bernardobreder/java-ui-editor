/*
 * 12/22/2008 DelegatingCellRenderer.java - A renderer for Completions that will delegate to the
 * Completion's provider's renderer, if there is one. This library is distributed under a modified
...
/**
 * List cell renderer that delegates to a {@link CompletionProvider}'s renderer, if it has one. If
 * it doesn't, it calls into a fallback renderer. If a fallback renderer isn't specified, it simply
 * renders <code>(({@link Completion})value).toString()</code>.
 *
...
class DelegatingCellRenderer extends DefaultListCellRenderer {
	...
	/**
	 * The renderer to fall back on if one isn't specified by a provider. This is usually
	 * <tt>this</tt>.
	 */
	 ...
	 *
	 * @param fallback The fallback cell renderer. If this is <code>null</code>, <tt>this</tt> will
	 *            be used.
