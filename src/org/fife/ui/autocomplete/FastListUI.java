/*
 * 04/26/2010 FastListUI.java - A JList UI implementation that computes the preferred size of all
 * cells really fast, to facilitate lists of possibly thousands of items rendered with HTML, which
 * is slow with BasicListUI extensions. This library is distributed under a modified BSD license.
...
/**
 * A custom list UI, used by the completion choices list. If the number of completion choices is
 * "large," it does a fast estimate of the preferred width and height of list items. This allows
 * HTML renderers to be used (such as {@link CompletionCellRenderer}), with thousands of completion
 * choices, with no performance penalty. With standard BasicListUI subclasses, this can cause very
 * poor performance <b>each time</b> the list is displayed, which is bad for lists that are
 * repeatedly hidden and re-displayed, such as completion choices. This is all because the
 * calculation to get the preferred size of each list item, when it is displayed with HTML, is slow.
 *
...
class FastListUI extends BasicListUI {
	...
	/**
	 * Whether the selection background was overridden (usually because of Nimbus) so we know to
	 * manually uninstall the color we installed.
	 */
	private boolean overriddenBackground;
	
	/**
	 * Whether the selection foreground was overridden (usually because of Nimbus) so we know to
	 * manually uninstall the color we installed.
	 */
	private boolean overriddenForeground;
	
	/**
	 * If there are more than this many completions in a single list, this UI will estimate the cell
	 * width and height needed for each item instead of computing it, for performance reasons.
	 */
	...
	/**
	 * Overridden to ensure we have selection background/foreground colors defined, even if we're in
	 * some weirdo LAF such as Nimbus which doesn't define them. Since FastListUI extends
	 * BasicListUI, we need these values to be defined.
	 */
	@Override
	...
	/**
	 * Recalculates the cell width and height of each cell in the list. This method is overridden to
	 * do a fast estimation if the completion list is too long, to improve performance for lists
	 * with huge amounts of completions.
	 */
	@Override
