/*
 * 04/26/2009 OutlineHighlightPainter.java - Highlight painter that draws an outline around its
 * text. This library is distributed under a modified BSD license. See the included
...
/**
 * Highlight painter that draws an outline around the text. This is used to draw bounds around
 ...
 */
/*
 * NOTE: Whenever you see text like "Workaround for Java Highlight issues", this is because
 * highlighted text in a JTextComponent gets "pushed" forward when the caret is at the Highlight's
 * start, when we need it to instead get prepended to. For this reason, the auto-complete package
 * adds its Highlights 1 char too long (1 char earlier than where it should really start), but only
...
class OutlineHighlightPainter extends DefaultHighlighter.DefaultHighlightPainter {
	 ...
	 * Constructor.
	 *
	 * @param color The color to draw the bounding boxes with. This cannot be <code>null</code>.
	 */
