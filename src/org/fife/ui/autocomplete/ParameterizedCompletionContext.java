/*
 * 06/17/2012 ParameritizedCompletionContext.java - Manages the state of parameterized
 * completion-related UI components during code completion. This library is distributed under a
...
/**
 * Manages UI and state specific to parameterized completions - the parameter description tool tip,
 * the parameter completion choices list, the actual highlights in the editor, etc. This component
 * installs new key bindings when appropriate to allow the user to cycle through the parameters of
 * the completion, and optionally cycle through completion choices for those parameters.
 *
...
class ParameterizedCompletionContext {
	...
	/**
	 * The minimum offset into the document that the caret can move to before this tool tip
	 * disappears.
	 */
	private int minPos;
	
	/**
	 * The maximum offset into the document that the caret can move to before this tool tip
	 * disappears.
	 */
	...
	/**
	 * The text before the caret for the current parameter. If {@link #paramChoicesWindow} is non-
	 * <code>null</code>, this is used to determine what parameter choices to actually show.
	 */
	 ...
	 * @return The text of the parameter containing the offset, or <code>null</code> if the offset
	 *         is not in a parameter.
	 */
	 ...
	 *
	 * @return The current parameter's highlight, or <code>null</code> if the caret is not in a
	 *         parameter's bounds.
	 ...
	 *
	 * @return The current parameter's starting offset, or <code>-1</code> if the caret is not in a
	 *         parameter's bounds.
	 ...
	 * highlights are ordered, sometimes they aren't (e.g. the "cursor" parameter in a template
	 * completion is always last, even though it can be anywhere in the template).
	 *
	 ...
	 *
	 * @return Whether the choice was inserted. This will be <code>false</code> if the window is not
	 *         visible, or no choice is selected.
	 */
	...
	/**
	 * Installs key bindings on the text component that facilitate the user editing this
	 * completion's parameters.
	 *
	...
	private void moveToNextParam() {
		...
		for (int i = 0; i < highlights.size(); i++) {
			...
			if (currentNext == null || currentNext.getStartOffset() < /* = */dot || (hl.getStartOffset() > dot && hl.getStartOffset() <= currentNext.getStartOffset())) {
				currentNext = hl;
				pos = i;
			}
		}
	...
	private void moveToPreviousParam() {
		...
		for (int i = 0; i < highlights.size(); i++) {
			Highlight h = highlights.get(i);
			if (currentPrev == null || currentPrev.getStartOffset() >= dot || (h.getStartOffset() < selStart && (h.getStartOffset() > currentPrev.getStartOffset() || pos == lastSelectedParam))) {
				currentPrev = h;
				pos = i;
			}
		}
	...
	/**
	 * Replaces highlighted text with new text. Takes special care so that the highlight stays just
	 * around the newly-highlighted text, since Swing's <code>Highlight</code> classes are funny
	 * about insertions at their start offsets.
	 *
	...
	/**
	 * Updates the text in the tool tip to have the current parameter displayed in bold. The
	 * "current parameter" is determined from the current caret position.
	 *
	...
	/**
	 * Updates the <code>LookAndFeel</code> of all popup windows this context manages.
	 */
	...
	/**
	 * Action performed when the user presses the up or down arrow keys and the parameter completion
	 * choices popup is visible.
	 */
