/*
 * 12/21/2008 Util.java - Utility methods for the autocompletion package. This library is
...
public class Util {
	...
	/**
	 * If a system property is defined with this name and set, ignoring case, to <code>true</code>,
	 * this library will not attempt to use Substance renderers. Otherwise, if a Substance Look and
	 * Feel is installed, we will attempt to use Substance cell renderers in all of our dropdowns.
	 * <p>
	 * Note that we do not have a build dependency on Substance, so all access to Substance stuff is
	 * done via reflection. We will fall back onto default renderers if something goes horribly
	 * wrong.
	 */
	public static final String PROPERTY_DONT_USE_SUBSTANCE_RENDERERS = "org.fife.ui.autocomplete.DontUseSubstanceRenderers";
	
	/**
	 * If this system property is <code>true</code>, then even the "main" two auto-complete windows
	 * will allow window decorations via {@link PopupWindowDecorator}. If this property is undefined
	 * or <code>false</code>, they won't honor such decorations. This is due to certain performance
	 * issues with translucent windows (used for drop shadows), even as of Java 7u2.
	 */
	 ...
	 *
	 * @param uri The URI to open. If this is <code>null</code>, nothing happens and this method
	 *            returns <code>false</code>.
	 * @return Whether the operation was successful. This will be <code>false</code> on JRE's older
	 *         than 1.6.
	 */
	...
	/**
	 * Returns the singleton <code>java.awt.Desktop</code> instance, or <code>null</code> if it is
	 * unsupported on this platform (or the JRE is older than 1.6).
	 *
	...
	/**
	 * Give apps a chance to decorate us with drop shadows, etc. Since very scrolly things such as
	 * lists (of e.g. completions) are *very* slow when in per-pixel translucent windows, even as of
	 * Java 7u2, we force the user to specify an extra option for the two "main" auto-complete
	 * windows.
	 *
	...
	/**
	 * Returns whether we should attempt to use Substance cell renderers and styles for things such
	 * as completion choices, if a Substance Look and Feel is installed. If this is
	 * <code>false</code>, we'll use our standard rendering for completions, even when Substance is
	 * being used.
	 *
	...
	/**
	 * Returns whether <code>str</code> starts with <code>start</code>, ignoring case.
	 *
	 * @param str The string to check.
	 * @param start The prefix to check for.
	 * @return Whether <code>str</code> starts with <code>start</code>, ignoring case.
	 */
	...
	/**
	 * Strips any HTML from a string. The string must start with " <code>&lt;html&gt;</code>" for
	 * markup tags to be stripped.
	 *
