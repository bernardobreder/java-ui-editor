...
/**
 * Base class for JFlex-based token makers using C-style syntax. This class knows how to:
 * <ul>
...
public abstract class AbstractJFlexCTokenMaker extends AbstractJFlexTokenMaker {
	...
	/**
	 * Creates and returns the action to use when the user inserts a newline. The default
	 * implementation intelligently closes multi-line comments. Subclasses can override.
	 *
	 * @return The action.
	...
	/**
	 * Returns <code>true</code> always as C-style languages use curly braces to denote code blocks.
	 *
	...
	/**
	 * Returns an action to handle "insert break" key presses (i.e. Enter). An action is returned
	 * that handles newlines differently in multi-line comments.
	 *
	 * @return The action.
	 */
	@Override
	...
	/**
	 * Returns whether a given token is an internal token type that represents an MLC or
	 * documentation comment continuing on to the next line. This is done by languages such as
	 * JavaScript that are a little more verbose than necessary so that their code can be
	 * copy-and-pasted into other <code>TokenMaker</code>s that use them as nested languages (such
	 * as HTML, JSP, etc.).
	 *
	 * @param t The token to check. This cannot be <code>null</code>.
	 * @return Whether the token is an internal token representing the end of a line for an MLC/doc
	 *         comment continuing on to the next line.
	 */
	...
	/**
	 * Action that knows how to special-case inserting a newline in a multi-line comment for
	...
	protected class CStyleInsertBreakAction extends RSyntaxTextAreaEditorKit.InsertBreakAction {
		...
		/**
		 * Returns whether the MLC token containing <code>offs</code> appears to have a "nested"
		 * comment (i.e., contains "<code>/*</code>" somewhere inside of it). This implies that it
		 * is likely a "new" MLC and needs to be closed. While not foolproof, this is usually good
		 * enough of a sign.
		 *
		 * @param textArea
		 * @param line
