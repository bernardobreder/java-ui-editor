...
/**
 * This class generates tokens for a {@link TokenMaker}. This class is here because it reuses tokens
 * when they aren't needed anymore to prevent This class doesn't actually create new tokens every
 * time <code>createToken</code> is called. Instead, it internally keeps a stack of available
 * already-created tokens. When more tokens are needed to properly display a line, more tokens are
 * added to the available stack. This saves from needless repetitive memory allocation. However, it
 * makes it IMPERATIVE that users call <code>resetTokenList</code> when creating a new token list so
...
class DefaultTokenFactory implements TokenFactory {
	...
	/**
	 * Adds tokens to the internal token list. This is called whenever a request is made and no more
	 ...
	 * <code>TokenMaker</code> every time a token list is generated for a new line so the tokens can
	 * be reused.
	 */
	@Override
