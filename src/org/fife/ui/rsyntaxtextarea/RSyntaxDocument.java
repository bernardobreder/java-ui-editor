 ...
 * <code>javax.swing.text.PlainDocument</code> except that it also keeps track of syntax
 * highlighting in the document. It has a "style" attribute associated with it that determines how
 * syntax highlighting is done (i.e., what language is being highlighted).
 * <p>
 * Instances of <code>RSyntaxTextArea</code> will only accept instances of
 * <code>RSyntaxDocument</code>, since it is this document that keeps track of syntax highlighting.
 * All others will cause an exception to be thrown.
 * <p>
 * To change the language being syntax highlighted at any time, you merely have to call
 * {@link #setSyntaxStyle}. Other than that, this document can be treated like any other save one
 * caveat: all <code>DocumentEvent</code>s of type <code>CHANGE</code> use their offset and length
 * values to represent the first and last lines, respectively, that have had their syntax coloring
 * change. This is really a hack to increase the speed of the painting code and should really be
 * corrected, but oh well.
 *
...
public class RSyntaxDocument extends RDocument implements Iterable<Token>, SyntaxConstants {
	...
	/**
	 * Array of values representing the "last token type" on each line. This is used in cases such
	 * as multi-line comments: if the previous line ended with an (unclosed) multi-line comment, we
	 * can use this knowledge and start the current line's syntax highlighting in multi-line comment
	 * state.
	 */
	...
	/**
	 * If this is set to <code>true</code>, debug information about how much token caching is
	 * helping is printed to stdout.
	 */
	...
	/**
	 * Alerts all listeners to this document of an insertion. This is overridden so we can update
	 * our syntax highlighting stuff.
	 * <p>
	...
	protected void fireInsertUpdate(DocumentEvent e) {
		...
		/*
		 * Now that the text is actually inserted into the content and element structure, we can
		 * update our token elements and "last tokens on lines" structure.
		 */
	 ...
	 * <code>postRemoveUpdate</code>) as this method is called in response to undo/redo events,
	 * whereas <code>postRemoveUpdate</code> is not.
	 * <p>
	 * Now that the text is actually inserted into the content and element structure, we can update
	...
	/**
	 * Returns the closest {@link TokenTypes "standard" token type} for a given "internal" token
	 * type (e.g. one whose value is <code>&lt; 0</code>).
	 *
	 * @param type The token type.
	 * @return The closest "standard" token type. If a mapping is not defined for this language,
	 *         then <code>type</code> is returned.
	 */
	...
	/**
	 * Returns whether closing markup tags should be automatically completed. This method only
	 * returns <code>true</code> if {@link #getLanguageIsMarkup()} also returns <code>true</code>.
	 *
	...
	/**
	 * Returns whether the current programming language uses curly braces (' <code>{</code>' and '
	 * <code>}</code>') to denote code blocks.
	 *
	 * @param languageIndex The language index at the offset in question. Since some
	 *            <code>TokenMaker</code>s effectively have nested languages (such as JavaScript in
	 *            HTML), this parameter tells the <code>TokenMaker</code> what sub-language to look
	 *            at.
	 ...
	 * @return The token type of the last token on the specified line. If the line is invalid, an
	 *         exception is thrown.
	 */
	...
	/**
	 * Returns the text to place at the beginning and end of a line to "comment" it in this
	 * programming language.
	 *
	 * @return The start and end strings to add to a line to "comment" it out. A <code>null</code>
	 *         value for either means there is no string to add for that part. A value of
	 *         <code>null</code> for the array means this language does not support
	...
	/**
	 * Returns a token list for the specified segment of text representing the specified line
	 * number. This method is basically a wrapper for <code>tokenMaker.getTokenList</code> that
	 * takes into account the last token on the previous line to assure token accuracy.
	 *
	 ...
	 * This is used to identify such things as the bounds of the "word" to select on
	 * double-clicking.
	 *
	...
	/**
	 * Returns an iterator over the paintable tokens in this document. Results are undefined if this
	 * document is modified while the iterator is being iterated through, so this should only be
	 * used on the EDT.
	 * <p>
	 ...
	 * supported by a document is determined by its {@link TokenMakerFactory}. By default, all
	 * <code>RSyntaxDocument</code>s support all languages built into <code>RSyntaxTextArea</code>.
	 *
	 * @param styleKey The new style to use, such as {@link SyntaxConstants#SYNTAX_STYLE_JAVA}. If
	 *            this style is not known or supported by this document, then
	...
	/**
	 * Sets the syntax style being used for syntax highlighting in this document. You should call
	 * this method if you've created a custom token maker for a language not normally supported by
	...
	/**
	 * Loops through the last-tokens-on-lines array from a specified point onward, updating
	 * last-token values until they stop changing. This should be called when lines are
	 * updated/inserted/removed, as doing so may cause lines below to change color.
	 *
	...
	/**
	 * Updates internal state information; e.g. the "last tokens on lines" data. After this, a
	 * changed update is fired to let listeners know that the document's structure has changed.
	 * <p>
