/*
 * 08/29/2004 RSyntaxTextAreaEditorKit.java - The editor kit used by RSyntaxTextArea. This library
...
/**
 * An extension of <code>RTextAreaEditorKit</code> that adds functionality for programming-specific
 * stuff. There are currently subclasses to handle:
 * <ul>
 * <li>Toggling code folds.</li>
 * <li>Aligning "closing" curly braces with their matches, if the current programming language uses
 * curly braces to identify code blocks.</li>
...
public class RSyntaxTextAreaEditorKit extends RTextAreaEditorKit {
	 ...
	 */
	private static final Action[] defaultActions = { new CloseCurlyBraceAction(), new CloseMarkupTagAction(), new BeginWordAction(beginWordAction, false), new BeginWordAction(selectionBeginWordAction, true), new ChangeFoldStateAction(rstaCollapseFoldAction, true), new ChangeFoldStateAction(rstaExpandFoldAction, false), new CollapseAllFoldsAction(), new CopyAsRtfAction(),
			// new DecreaseFontSizeAction(),
	new DecreaseIndentAction(), new DeletePrevWordAction(), new EndAction(endAction, false), new EndAction(selectionEndAction, true), new EndWordAction(endWordAction, false), new EndWordAction(endWordAction, true), new ExpandAllFoldsAction(), new GoToMatchingBracketAction(), new InsertBreakAction(),
			// new IncreaseFontSizeAction(),
	new InsertTabAction(), new NextWordAction(nextWordAction, false), new NextWordAction(selectionNextWordAction, true), new PossiblyInsertTemplateAction(), new PreviousWordAction(previousWordAction, false), new PreviousWordAction(selectionPreviousWordAction, true), new SelectWordAction(), new ToggleCommentAction(), };
	...
	/**
	 * Fetches the set of commands that can be used on a text component that is using a model and
	...
	/**
	 * Returns localized text for an action. There's definitely a better place for this
	 * functionality.
	 *
	...
	/**
	 * Positions the caret at the beginning of the word. This class is here to better handle finding
	 * the "beginning of the word" for programming languages.
	 */
	...
	/**
	 * Action that (optionally) aligns a closing curly brace with the line containing its matching
	 * opening curly brace.
	 */
	...
	public static class CloseMarkupTagAction extends RecordableTextAction {
		 ...
		 * @param dot The location of the caret. This should be right after the start of a closing
		 *            tag token (e.g. "<code>&lt;/</code>" or " <code>[</code>" in the case of
		 *            BBCode).
	...
	/**
	 * Action for when un-indenting lines (either the current line if there is selection, or all
	 * selected lines if there is one).
	 */
	public static class DecreaseIndentAction extends RecordableTextAction {
		...
		/**
		 * Actually does the "de-indentation." This method finds where the given element's leading
		 * whitespace ends, then, if there is indeed leading whitespace, removes either the last
		 * char in it (if it is a tab), or removes up to the number of spaces equal to a tab in the
		 * specified document (i.e., if the tab size was 5 and there were 3 spaces at the end of the
		 * leading whitespace, the three will be removed; if there were 8 spaces, only the first 5
		 * would be removed).
		 *
	...
	/**
	 * Deletes the previous word, but differentiates symbols from "words" to match the behavior of
	 * code editors.
	 */
	...
	/**
	 * Positions the caret at the end of the word. This class is here to better handle finding the
	 * "end of the word" in programming languages.
	 */
	...
	static abstract class FoldRelatedAction extends RecordableTextAction {
		...
		public FoldRelatedAction(String name, Icon icon, String desc, Integer mnemonic, KeyStroke accelerator) {
			super(name, icon, desc, mnemonic, accelerator);
		}
	...
	/**
	 * Action for moving the caret to the "matching bracket" of the bracket at the caret position
	...
	/**
	 * Action for when the user presses the Enter key. This is here so we can be smart and
	 * "auto-indent" for programming languages.
	 */
	public static class InsertBreakAction extends RTextAreaEditorKit.InsertBreakAction {
		...
		/**
		 * @return The first location in the string past <code>pos</code> that is NOT a whitespace
		 *         char, or <code>-1</code> if only whitespace chars follow <code>pos</code> (or it
		 *         is the end position in the string).
		 */
		...
		private static final int getOpenBraceCount(RSyntaxDocument doc, int languageIndex) {
			int openCount = 0;
			for (Token t : doc) {
		...
		/**
		 * Actually inserts the newline into the document, and auto-indents if appropriate. This
		 * method can be called by token makers who implement a custom action for inserting
		 * newlines.
		 *
		 * @param textArea
	...
	/**
	 * Action for inserting tabs. This is extended to "block indent" a group of contiguous lines if
	 * they are selected.
	 */
	...
	/**
	 * Action to move the selection and/or caret. Constructor indicates direction to use. This class
	 * overrides the behavior defined in {@link RTextAreaEditorKit} to better skip "words" in source
	 * code.
	 */
	...
	/**
	 * Action for when the user tries to insert a template (that is, they've typed a template ID and
	 * pressed the trigger character (a space) in an attempt to do the substitution).
	 */
	...
	/**
	 * Selects the word around the caret. This class is here to better handle selecting "words" in
