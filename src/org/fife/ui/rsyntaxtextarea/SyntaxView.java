...
/**
 * The <code>javax.swing.text.View</code> object used by {@link RSyntaxTextArea} when word wrap is
 * disabled. It implements syntax highlighting for programming languages using the colors and font
 * styles specified by the <code>RSyntaxTextArea</code>.
 * <p>
 * You don't really have to do anything to use this class, as {@link RSyntaxTextAreaUI}
 * automatically sets the text area's view to be an instance of this class if word wrap is disabled.
 * <p>
...
public class SyntaxView extends View implements TabExpander, TokenOrientedView, RSTAView {
	...
	/**
	 * The default font used by the text area. If this changes we need to recalculate the longest
	 * line.
	 */
	private Font font;
	...
	/**
	 * The current longest line. This is used to calculate the preferred width of the view. Since
	 * the calculation is potentially expensive, we try to avoid it by stashing which line is
	...
	/**
	 * Temporary token used when we need to "modify" tokens for rendering purposes. Since tokens
	 * returned from RSyntaxDocuments are treated as immutable, we use this temporary token to do
	 * that work.
	 */
	...
	/**
	 * Draws the passed-in text using syntax highlighting for the current language. It is assumed
	 * that the entire line is either not in a selected region, or painting with a
	 * selection-foreground color is turned off.
	 *
	...
	/**
	 * Draws the passed-in text using syntax highlighting for the current language. Tokens are
	 * checked for being in a selected region, and are rendered appropriately if they are.
	 *
	 * @param painter The painter to render the tokens.
	 * @param token The list of tokens to draw.
	 * @param g The graphics context in which to draw.
	...
	/**
	 * Provides a way to determine the next visually represented model location that one might place
	 * a caret. Some views may not be visible, they might not be in the same order found in the
	 ...
	 * specified offset into the document. Note that for this plain (non-wrapped) view, this is
	 * simply the token list for the logical line above the line containing <code>offset</code>,
	 ...
	 *         <code>offset</code> is in the first line in the document, <code>null</code> is
	 *         returned.
	 */
	@Override
	 ...
	 * specified offset into the document. Note that for this plain (non-wrapped) view, this is
	 * simply the token list for the logical line below the line containing <code>offset</code>,
	 * since lines are not wrapped.
	 *
	 * @param offset The offset in question.
	 * @return A token list for the physical (and in this view, logical) line after this one. If
	 *         <code>offset</code> is in the last physical line in the document, <code>null</code>
	 *         is returned.
	 */
	@Override
	...
	/**
	 * If the passed-in line is longer than the current longest line, then the longest line is
	 * updated.
	 *
	...
	/**
	 * Repaint the region of change covered by the given document event. Damages the line that
	 * begins the range to cover the case when the insert/remove is only on one line. If lines are
	 * added or removed, damages the whole view. The longest line is checked to see if it has
	 * changed.
	 */
