...
/**
 * An extension of {@link org.fife.ui.rsyntaxtextarea.RSyntaxTextArea} that adds information about
 * the file being edited, such as:
 * <ul>
 ...
 * {@link UnicodeWriter} class. Use {@link UnicodeWriter#setWriteUtf8BOM(boolean)} to toggle writing
 * BOMs for UTF-8 files.
 * <p>
 * Both local and remote files (e.g. ftp) are supported. See the {@link FileLocation} class for more
 * information.
 *
...
public class TextEditorPane extends RSyntaxTextArea implements DocumentListener {
	...
	/**
	 * The last time this file was modified on disk, for local files. For remote files, this value
	 * should always be {@link #LAST_MODIFIED_UNKNOWN}.
	 */
	 ...
	 * @param textMode Either <code>INSERT_MODE</code> or <code>OVERWRITE_MODE</code>.
	 * @param wordWrapEnabled Whether or not to use word wrap in this pane.
	 * @param loc The location of the text file being edited. If this value is <code>null</code>, a
	 *            file named "Untitled.txt" in the current directory is used.
	 * @throws IOException If an IO error occurs reading the file at <code>loc</code>. This of
	 *             course won't happen if <code>loc</code> is <code>null</code>.
	 */
	 ...
	 * @param wordWrapEnabled Whether or not to use word wrap in this pane.
	 * @param loc The location of the text file being edited. If this value is <code>null</code>, a
	 *            file named "Untitled.txt" in the current directory is used. This file is displayed
	 *            as empty even if it actually exists.
	 * @param defaultEnc The default encoding to use when opening the file, if the file is not
	 *            Unicode. If this value is <code>null</code>, a system default value is used.
	 * @throws IOException If an IO error occurs reading the file at <code>loc</code>. This of
	 *             course won't happen if <code>loc</code> is <code>null</code>.
	 */
	 ...
	 * <em>by this editor pane</em>. If the file has been modified on disk by another process after
	 * it was loaded into this editor pane, this method will not return the actual file's last
	 * modified time.
	 * <p>
	 * For remote files, this method will always return {@link #LAST_MODIFIED_UNKNOWN}.
	 *
	 * @return The timestamp when this file was last loaded or saved by this editor pane, if it is a
	 *         local file, or {@link #LAST_MODIFIED_UNKNOWN} if it is a remote file.
	 ...
	 * <code>\r\n</code>", or "<code>\r</code>").
	 * <p>
	 * Note that this value is an <code>Object</code> and not a <code>String</code> as that is the
	 * way the {@link Document} interface defines its property values. If you always use
	 * {@link #setLineSeparator(String)} to modify this value, then the value returned from this
	 * method will always be a <code>String</code>.
	 *
	 * @return The line separator. If this value is <code>null</code>, then the system default line
	 *         separator is used (usually the value of
	 ...
	 *
	 * @param loc The file location. If this is <code>null</code>, a default location is used and an
	 *            empty file is displayed.
	 * @param defaultEnc The default encoding to use when opening the file, if the file is not
	 *            Unicode. If this value is <code>null</code>, a system default value is used.
	 * @throws IOException If an IO error occurs reading from <code>loc</code>. If <code>loc</code>
	 *             is <code>null</code>, this cannot happen.
	 */
	...
	/**
	 * Returns whether the text file has been modified outside of this editor since the last load or
	 * save operation. Note that if this is a remote file, this method will always return
	 * <code>false</code>.
	 * <p>
	 * This method may be used by applications to implement a reloading feature, where the user is
	 * prompted to reload a file if it has been modified since their last open or save.
	 *
	...
	/**
	 * Loads the specified file in this editor. This method fires a property change event of type
	 * {@link #FULL_PATH_PROPERTY}.
	 *
	 * @param loc The location of the file to load. This cannot be <code>null</code>.
	 * @param defaultEnc The encoding to use when loading/saving the file. This encoding will only
	 *            be used if the file is not Unicode. If this value is <code>null</code>, the system
	...
	/**
	 * Reloads this file from disk. The file must exist for this operation to not throw an
	 * exception.
	 * <p>
	 * The file's "dirty" state will be set to <code>false</code> after this operation. If this is a
	 * local file, its "last modified" time is updated to reflect that of the actual file.
	 * <p>
	 * Note that if the file has been modified on disk, and is now a Unicode encoding when before it
	 * wasn't (or if it is a different Unicode now), this will cause this {@link TextEditorPane}'s
	 * encoding to change. Otherwise, the file's encoding will stay the same.
	 *
	 ...
	 * <p>
	 * The text area's "dirty" state is set to <code>false</code>, and if this is a local file, its
	...
	/**
	 * Sets whether or not this text in this editor has unsaved changes. This fires a property
	 * change event of type {@link #DIRTY_PROPERTY}.
	 * <p>
	 * Applications will usually have no need to call this method directly; the only time you might
	 * have a need to call this method directly is if you have to initialize an instance of
	 * TextEditorPane with content that does not come from a file. <code>TextEditorPane</code>
	 * automatically sets its own dirty flag when its content is edited, when its encoding is
	 * changed, or when its line ending property is changed. It is cleared whenever
	 * <code>load()</code>, <code>reload()</code>, <code>save()</code>, or <code>saveAs()</code> are
	 * called.
	 *
	...
	/**
	 * Sets the encoding to use when reading or writing this file. This method sets the editor's
	 * dirty flag when the encoding is changed.
	 *
	 ...
	 * <code>\r\n</code>" or "<code>\r</code>"). Besides parameter checking, this method is
	 * preferred over <code>getDocument().putProperty()</code> because it sets the editor's dirty
	 ...
	 * Sets the line separator sequence to use when this file is saved (e.g. " <code>\n</code>", "
	 * <code>\r\n</code>" or "<code>\r</code>"). Besides parameter checking, this method is
	 * preferred over <code>getDocument().putProperty()</code> because can set the editor's dirty
	 * flag when the line separator is changed.
	 *
	 * @param separator The new line separator.
	...
	/**
	 * Sets whether or not this text area should be treated as read-only. This fires a property
	 * change event of type {@link #READ_ONLY_PROPERTY}.
	 *
	...
	/**
	 * Syncs this text area's "last saved or loaded" time to that of the file being edited, if that
	 * file is local and exists. If the file is remote or is local but does not yet exist, nothing
	 * happens.
	 * <p>
	 * You normally do not have to call this method, as the "last saved or loaded" time for
	 * {@link TextEditorPane}s is kept up-to-date internally during such operations as
	 * {@link #save()}, {@link #reload()}, etc.
	 *
	 * @see #getLastSaveOrLoadTime()
	 * @see #isModifiedOutsideEditor()
	 */
