 ...
 * <p>
 * <b>Note:</b> The instances of <code>Token</code> returned by {@link RSyntaxDocument}s are pooled
 * and should always be treated as immutable. They should not be cast to <code>TokenImpl</code> and
 * modified. Modifying tokens you did not create yourself can and will result in rendering issues
 * and/or runtime exceptions. You have been warned!
 *
...
public class TokenImpl implements Token {
	...
	/**
	 * The text this token represents. This is implemented as a segment so we can point directly to
	 * the text in the document without having to make a copy of it.
	 */
	public char[] text;
	...
	/**
	 * Creates a "null token." The token itself is not null; rather, it signifies that it is the
	 * last token in a linked list of tokens and that it is not part of a "multi-line token."
	 */
	public TokenImpl() {
		this.text = null;
		this.textOffset = -1;
	...
	public boolean equals(Object obj) {
		...
		Token t2 = (Token) obj;
		return offset == t2.getOffset() && type == t2.getType() && languageIndex == t2.getLanguageIndex() && hyperlink == t2.isHyperlink() && ((getLexeme() == null && t2.getLexeme() == null) || (getLexeme() != null && getLexeme().equals(t2.getLexeme())));
	...
	/**
	 * Returns a <code>String</code> of the form "#xxxxxx" good for use in HTML, representing the
	 * given color.
	 *
	 * @param color The color to get a string for.
	 * @return The HTML form of the color. If <code>color</code> is <code>null</code>,
	 *         <code>#000000</code> is returned.
	 */
	 ...
	 * <p>
	 * <b>Note:</b> You should not modify <code>Token</code> instances you did not create yourself
	 * (e.g., came from an <code>RSyntaxDocument</code>). If you do, rendering issues and/or runtime
	 * exceptions will likely occur. You have been warned!
	 *
	 * @param pos The offset into the document this token should start at. Note that this token must
	 *            already contain this position; if it doesn't, an exception is thrown.
	 * @throws IllegalArgumentException If pos is not already contained by this token.
	 * @see #moveOffset(int)
	 */
	 ...
	 * exceptions will likely occur. You have been warned!
	 *
	 * @param amt The amount to move the starting offset. This should be between <code>0</code> and
	 *            <code>textCount</code>, inclusive.
	...
	/**
	 * Sets the value of this token to a particular segment of a document. The "next token" value is
	 * cleared.
	 *
	 * @param line The segment from which to get the token.
	 * @param beg The first character's position in <code>line</code>.
