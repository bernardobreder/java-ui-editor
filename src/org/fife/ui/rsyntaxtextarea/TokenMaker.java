...
/**
 * An implementation of <code>TokenMaker</code> is a class that turns text into a linked list of
 * <code>Token</code>s for syntax highlighting in a particular language.
 *
 * @see Token
...
public interface TokenMaker {
	...
	/**
	 * Adds a null token to the end of the current linked list of tokens. This should be put at the
	 * end of the linked list whenever the last token on the current line is NOT a multi-line token.
	 */
	 ...
	 * {@link Token#NULL} if it was finished. For example, if C-style syntax highlighting is being
	 * implemented, and <code>text</code> contained a line of code that contained the beginning of a
	 * comment but no end-comment marker ("*\/"), then this method would return
	 * {@link Token#COMMENT_MULTILINE} for that line. This is useful for doing syntax highlighting.
	 *
	 * @param text The line of tokens to examine.
	 * @param initialTokenType The token type to start with (i.e., the value of
	 *            <code>getLastTokenTypeOnLine</code> for the line before <code>text</code>).
	...
	/**
	 * Returns the object in charge of marking all occurrences of the token at the current caret
	 * position, if it is a relevant token. If <code>null</code> is returned, a default
	 * <code>OccurrenceMarker</code> is used.
	 *
	...
	/**
	 * If a line ends in the specified token, this method returns whether a new line inserted after
	 * that line should be indented.
	 *
