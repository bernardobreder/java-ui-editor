...
public abstract class TokenMakerBase implements TokenMaker {
	...
	/**
	 * "0" implies this is the "main" language being highlighted. Positive values imply various
	 * "secondary" or "embedded" languages, such as CSS or JavaScript in HTML. While this value is
	 * non-zero, tokens will be generated with this language index so they can (possibly) be painted
	 * differently, so "embedded" languages can be rendered with a special background.
	 */
	...
	/**
	 * Returns the occurrence marker to use for this token maker. Subclasses can override to use
	 ...
	 * <p>
	 * The default implementation returns <code>type</code> always, which denotes that a mapping
	 * from internal token types to standard token types is not defined; subclasses can override.
	 *
	...
	/**
	 * Returns whether this programming language uses curly braces (' <code>{</code>' and '
	 * <code>}</code>') to denote code blocks. The default implementation returns <code>false</code>
	...
	/**
	 * Returns an action to handle "insert break" key presses (i.e. Enter). The default
	 * implementation returns <code>null</code>. Subclasses can override.
	 *
	 ...
	 * current programming language. The default implementation returns true if <tt>type</tt> is
	 * {@link Token#IDENTIFIER}. Subclasses can override this method to support other token types,
	 * such as {@link Token#VARIABLE}.
	 *
	 * @param type The token type.
	...
	/**
	 * The default implementation returns <code>false</code> always. Languages that wish to better
	 * support auto-indentation can override this method.
	 *
	 ...
	 * The default implementation returns <code>true</code> for letters, numbers, and certain
	 * symbols.
	 */
	@Override
	...
	/**
	 * The default implementation returns <code>false</code> always. Subclasses that are
	 * highlighting a markup language should override this method to return <code>true</code>.
	 *
	...
	/**
	 * Deletes the linked list of tokens so we can begin anew. This should never have to be called
	 * by the programmer, as it is automatically called whenever the user calls
	 * {@link #getLastTokenTypeOnLine(Segment, int)} or {@link #getTokenList(Segment, int, int)}.
	 */
	...
	/**
	 * Sets the language index to assign to tokens moving forward. This property is used to
	 * designate tokens as being in "secondary" languages (such as CSS or JavaScript in HTML).
	 *
