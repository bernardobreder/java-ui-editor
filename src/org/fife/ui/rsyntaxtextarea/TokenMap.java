...
/**
 * A hash table for reserved words, etc. defined by a {@link TokenMaker}. This class is designed for
 * the quick lookup of tokens, as it can compare <code>Segment</code>s without the need to allocate
 * a new string.
 * <p>
 * The <code>org.fife.ui.rsyntaxtextarea</code> package uses this class to help identify reserved
 * words in programming languages. An instance of {@link TokenMaker} will create and initialize an
 * instance of this class containing all reserved words, data types, and all other words that need
 * to be syntax-highlighted for that particular language. When the token maker parses a line and
 * identifies an individual token, it is looked up in the <code>TokenMap</code> to see if it should
...
public class TokenMap {
	 ...
	 *
	 * @param ignoreCase Whether or not this token map should ignore case when comparing tokens.
	 */
	...
	/**
	 * Returns the token type associated with the given text, if the given text is in this token
	 * map. If it isn't, <code>-1</code> is returned.
	 *
	 ...
	 * @return The token type associated with the given text, or <code>-1</code> if this token was
	 *         not specified in this map.
	 */
	...
	public int get(char[] array1, int start, int end) {
		...
		/*
		 * We check whether or not to ignore case before doing any looping to minimize the number of
		 * extraneous comparisons we do. This makes for slightly redundant code, but it'll be a
	...
	/**
	 * Returns whether this token map ignores case when checking for tokens. This property is set in
	 * the constructor and cannot be changed, as this is an intrinsic property of a particular
	...
	/**
	 * Adds a string to this token map. The char array passed-in will be used as the actual data for
	 * the token, so it may well be modified (such as lower-casing it if <code>ignoreCase</code> is
	 * <code>true</code>). This shouldn't be an issue though as this method is only called from the
	...
	/**
	 * The "token" used by a token map. Note that this isn't the same thing as the {@link Token}
	 * class, but it's basically a 1-1 correspondence for reserved words, etc.
	 */
