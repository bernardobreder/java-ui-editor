...
public class TokenUtils {
	 ...
	 * </pre>
	 *
	 * and you used this method to make the token list start at position 44, then the token list
	 * would be modified to be the following:
	 *
	 * <pre>
	 ...
	 * </pre>
	 *
	 * Tokens that come before the specified position are forever lost, and the token containing
	 * that position is made to begin at that position if necessary. All token types remain the same
	 * as they were originally.
	 * <p>
	 * This method can be useful if you are only interested in part of a token list (i.e., the line
	 * it represents), but you don't want to modify the token list yourself.
	 *
	 * @param tokenList The list to make start at the specified position. This parameter is
	 *            modified.
	 * @param pos The position at which the new token list is to start. If this position is not in
	 *            the passed-in token list, returned token list will either be <code>null</code> or
	 *            the unpaintable token(s) at the end of the passed-in token list.
	 ...
	 * @return Information about the "sub" token list. This will be <code>null</code> if
	 *         <code>pos</code> was not a valid offset into the token list.
	 ...
	 *            <code>null</code> but callers can pass in a "buffer" token for performance if
	 *            desired.
	 * @return Information about the "sub" token list. This will be <code>null</code> if
	 *         <code>pos</code> was not a valid offset into the token list.
	 * @see #getSubTokenList(Token, int, TabExpander, RSyntaxTextArea, float)
	 */
	public static TokenSubList getSubTokenList(Token tokenList, int pos, TabExpander e, final RSyntaxTextArea textArea, float x0, TokenImpl tempToken) {
