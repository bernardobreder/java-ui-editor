 ...
 * <li>If a space was found, all characters up to and including it are painted (it is painted with a
 * special symbol to denote it as a space).</li>
 ...
 * </ul>
 * This means that rendering hints are applied to all groups of characters within a token, excluding
 * whitespace and tabs.
 * <p>
 * A problem with this implementation is that FontMetrics.charsWidth() is still used to calculate
 * the width of a group of chars painted. Thus, the group of characters will be painted with the
 * rendering hints specified, but the following tab (or group of characters if the current group was
 * the end of a token) will not necessarily be painted at the proper x-coordinate (as
 * FontMetrics.charsWidth() returns an <code>int</code> and not a <code>float</code>). The way
 * around this would be to calculate the token's width in such a way that a float is returned
