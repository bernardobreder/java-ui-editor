...
/**
 * A basic fold parser that can be used for languages such as C, that use curly braces to denote
 * code blocks. This parser searches for curly brace pairs and creates code folds out of them. It
 * can also optionally find C-style multi-line comments ("<code>/* ... *&#47;</code>") and make them
 * foldable as well.
 * <p>
 * This parser knows nothing about language semantics; it uses <code>RSyntaxTextArea</code>'s syntax
 * highlighting tokens to identify curly braces. By default, it looks for single-char tokens of type
 * {@link Token#SEPARATOR}, with lexemes '<code>{</code>' or '<code>}</code>'. If your
 * {@link TokenMaker} uses a different token type for curly braces, you should override the
 * {@link #isLeftCurly(Token)} and {@link #isRightCurly(Token)} methods with your own definitions.
 * In theory, you could extend this fold parser to parse languages that use completely different
 * tokens than curly braces to denote foldable regions by overriding those two methods.
 * <p>
 * Note also that this class may impose somewhat of a performance penalty on large source files,
 * since it re-parses the entire document each time folds are reevaluated.
 *
...
public class CurlyFoldParser implements FoldParser {
	...
	/**
	 * Creates a fold parser that identifies foldable regions via curly braces as well as C-style
	 * multi-line comments.
	 */
	 ...
	 * Constructor.
	 *
	 * @param cStyleMultiLineComments Whether to scan for C-style multi-line comments and make them
	 *            foldable.
	 * @param java Whether this parser is folding Java. This adds extra parsing rules, such as
	 *            grouping all import statements into a fold section.
	 */
	...
	/**
	 * Returns whether the token is a left curly brace. This method exists so subclasses can provide
	 * their own curly brace definition.
	 *
	 * @param t The token.
	...
	/**
	 * Returns whether the token is a right curly brace. This method exists so subclasses can
	 * provide their own curly brace definition.
	 *
	 * @param t The token.
