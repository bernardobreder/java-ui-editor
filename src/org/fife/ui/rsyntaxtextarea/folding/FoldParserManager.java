...
/**
 * Manages fold parsers. Instances of <code>RSyntaxTextArea</code> call into this class to retrieve
 * fold parsers for whatever language they're editing. Folks implementing custom languages can add a
 * {@link FoldParser} implementation for their language to this manager and it will be used by RSTA.
 *
...
public class FoldParserManager implements SyntaxConstants {
	...
	/**
	 * Adds a mapping from a syntax style to a fold parser. The parser specified will be shared
	 * among all RSTA instances editing that language, so it should be stateless (which should not
	 * be difficult for a fold parser). You can also override the fold parser for built-in
	 * languages, such as <code>SYNTAX_STYLE_JAVA</code>, with your own parser implementations.
	 *
	 ...
	 * @return A fold parser to use, or <code>null</code> if none is registered for the language.
	 */
