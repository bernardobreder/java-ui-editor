...
/**
 * Fold parser for HTML 5, PHP and JSP. For HTML, we currently don't fold <em>everything</em>
 * possible, just the "big" stuff. For PHP, we only fold the "big" HTML stuff and PHP regions, not
 * code blocks in the actual PHP. For JSP we only fold the "big" HTML stuff and JSP blocks, not
 * anything in the actual Java code.
 *
...
public class HtmlFoldParser implements FoldParser {
	...
	/**
	 * The set of tags we allow to be folded. These are tags that must have explicit close tags in
	...
	/**
	 * Grabs the token representing the closing of a tag (i.e. "<code>&gt;</code> " or "
	 * <code>/&gt;</code>"). This should only be called after a tag name has been parsed, to ensure
	 * the "closing" of other tags is not identified.
	 *
	 ...
	 * @param info On return, information about the closing of the tag is returned in this object.
	 */
	...
	/**
	 * Returns whether a closing tag ("<code>&lt;/...&gt;</code>") with a specific name is the
	 * closing tag of our current fold region.
	 *
	...
	/**
	 * If this fold has a parent fold, this method removes it from its parent. Otherwise, it's
	 * assumed to be the most recent (top-level) fold in the <code>folds</code> list, and is removed
	 * from that.
	 *
	...
	/**
	 * A simple wrapper for the token denoting the closing of a tag (i.e. " <code>&gt;</code>" or "
	 * <code>/&gt;</code>").
	 */
