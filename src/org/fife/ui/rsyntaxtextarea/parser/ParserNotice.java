 ...
 * <p>
 * Since different parsers have different levels of precision when it comes to identifying errors in
 * code, this class supports marking parser notices on either a per-line basis or arbitrary regions
 * of a document. For any <code>ParserNotice</code>, {@link #getLine()} is guaranteed to return the
 * (primary) line containing the notice, but {@link #getOffset()} and {@link #getLength()} are
 * allowed to return <code>-1</code> if that particular notice isn't mapped to a specific region of
 * code. Applications can check whether an instance of this class only has line-level information
 * with the
 *
...
public interface ParserNotice extends Comparable<ParserNotice> {
	 ...
	 * @return Whether the position is contained. This will always return <code>false</code> if
	 *         {@link #getOffset()} returns <code>-1</code>.
	 */
	 ...
	 *
	 * @return The length of the code the message is concerned with, or <code>-1</code> if unknown.
	 * @see #getOffset()
	 * @see #getLine()
	 */
	...
	/**
	 * Returns whether this parser notice has offset and length information (as opposed to just what
	 * line number to mark).
	 *
	 ...
	 *
	 * @return The tool tip text. If none has been explicitly set, this method returns the same text
	 *         as {@link #getMessage()}.
	 */
