...
/**
 * Parser that identifies "task tags," such as "<code>TODO</code>", " <code>FIXME</code>", etc. in
...
public class TaskTagParser extends AbstractParser {
	...
	/**
	 * Creates a new task parser. The default parser treats the following identifiers in comments as
	 * task definitions: "<code>TODO</code>", " <code>FIXME</code>", and "<code>HACK</code>".
	 */
	 ...
	 *
	 * @return The regular expression. This may be <code>null</code> if no regular expression was
	 *         specified (or an empty string was specified).
	...
	/**
	 * Sets the pattern of task identifiers. You will usually want this to be a list of words
	 * "or'ed" together, such as " <code>TODO|FIXME|HACK|REMIND</code>".
	 *
	 * @param pattern The pattern. A value of <code>null</code> or an empty string effectively
	 *            disables task parsing.
	 * @throws PatternSyntaxException If <code>pattern</code> is an invalid regular expression.
	 * @see #getTaskPattern()
	 */
	...
	/**
	 * A parser notice that signifies a task. This class is here so we can treat tasks specially and
	 * show them in the {@link ErrorStrip} even though they are <code>INFO</code>-level and marked
	 * as "don't show in editor."
	 */
