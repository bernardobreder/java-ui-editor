...
/**
 * A parser for XML documents. Adds squiggle underlines for any XML errors found (though most XML
 * parsers don't really have error recovery and so only can find one error at a time).
 * <p>
 * This class isn't actually used by RSyntaxTextArea anywhere, but you can install and use it
 * yourself. Doing so is as simple as:
 *
 * <pre>
 ...
 * </pre>
 *
 * To support DTD validation, specify an entity resolver when creating the parser, and enable
 * validation like so:
 *
 * <pre>
 ...
 * </pre>
 *
 * Also note that a single instance of this class can be installed on multiple instances of
 * <code>RSyntaxTextArea</code>.
 * <p>
 * For a more complete XML parsing/validation solution, see the <a
 * href="https://github.com/bobbylight/RSTALanguageSupport">RSTALanguageSupport project</a>'s
 * <code>XmlLanguageSupport</code> class.
 *
...
public class XmlParser extends AbstractParser {
	 ...
	 *
	 * @param validating Whether DTD validation should be enabled. If this is <code>true</code>,
	 *            documents must specify a DOCTYPE, and you should have used the constructor
	 *            specifying an entity resolver.
	 * @see #isValidating()
	 */
	 ...
	 * textArea.setSyntaxEditingStyle("text/xml"); XmlParser parser = new XmlParser(new
	 * EntityResolver() { public InputSource resolveEntity(String publicId, String systemId) throws
	 * SAXException, IOException { if
	 * ("http://fifesoft.com/rsyntaxtextarea/theme.dtd".equals(systemId)) { return new
	 * org.xml.sax.InputSource(getClass().getResourceAsStream("/theme.dtd")); } return null; } });
	 * parser.setValidating(true); textArea.addParser(parser); try { textArea.read(new
	 * java.io.BufferedReader(new java.io.FileReader("C:/temp/test.xml")), null); } catch (Exception
	 * e) { e.printStackTrace(); } frame.setContentPane(new
	...
	/**
	 * Callback notified when errors are found in the XML document. Adds a notice to be
	 * squiggle-underlined.
	 */
