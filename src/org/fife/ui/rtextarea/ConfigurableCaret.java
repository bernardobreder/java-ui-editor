...
/**
 * The caret used by {@link RTextArea}. This caret has all of the properties that
 * <code>javax.swing.text.DefaultCaret</code> does, as well as adding the following niceties:
 * <ul>
 * <li>This caret can render itself many different ways; see the {@link #setStyle(CaretStyle)}
 * method and {@link CaretStyle} for more information.</li>
 * <li>On Microsoft Windows and other operating systems that do not support system selection (i.e.,
 * selecting text, then pasting via the middle mouse button), clicking the middle mouse button will
 * cause a regular paste operation to occur. On systems that support system selection (i.e., all
 * UNIX variants), the middle mouse button will behave normally.</li>
 * </ul>
 *
...
public class ConfigurableCaret extends DefaultCaret {
	...
	/**
	 * The selection painter. By default this paints selections with the text area's selection
	 * color.
	 */
	 ...
	 *
	 * @param style The style to use when painting the caret. If this is invalid, then
	...
	/**
	 * Gets the painter for the Highlighter. This is overridden to return our custom selection
	 * painter.
	 *
	 * @return The painter.
	 */
	 ...
	 *
	 * @param c The text component. If this is not an {@link RTextArea}, an <code>Exception</code>
	 *            will be thrown.
	 * @see Caret#install
	 */
	@Override
	...
	/**
	 * Returns whether this caret is always visible (as opposed to blinking, or not visible when the
	 * editor's window is not focused). This can be used by popup windows that want the caret's
	...
	/**
	 * Toggles whether this caret should always be visible (as opposed to blinking, or not visible
	 * when the editor's window is not focused). This can be used by popup windows that want the
	 * caret's location to still be visible for contextual purposes while they are displayed.
	 *
	 ...
	 *
	 * @param visible Whether the selection should be visible. This parameter is ignored.
	 */
	@Override
	...
	/**
	 * Helper function used by the block and underline carets to ensure the width of the painted
	 * caret is valid. This is done for the following reasons:
	 * <ul>
	 * <li>The <code>View</code> classes in the javax.swing.text package always return a width of
	 * "1" when <code>modelToView</code> is called. We'll be needing the actual width.</li>
	 * <li>Even in smart views, such as <code>RSyntaxTextArea</code>'s <code>SyntaxView</code> and
	 * <code>WrappedSyntaxView</code> that return the width of the current character, if the caret
	 * is at the end of a line for example, the width returned from <code>modelToView</code> will be
	 * 0 (as the width of unprintable characters such as '\n' is calculated as 0). In this case,
	 * we'll use a default width value.</li>
	 * </ul>
	 *
	...
	/**
	 * Keeps the caret out of folded regions in edge cases where it doesn't happen automatically,
	 * for example, when the caret moves automatically in response to Document.insert() and
	 * Document.remove() calls. Most keyboard shortcuts already take folding into account, as do
	 * viewToModel() and modelToView(), so this filter usually does not do anything.
	 * <p>
