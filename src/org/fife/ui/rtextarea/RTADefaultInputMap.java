...
/**
 * The default input map for an <code>RTextArea</code>. For the most part it is exactly that the one
 * for a <code>JTextArea</code>, but it adds a few things. Currently, the new key bindings include:
 * <ul>
 * <li>HOME key toggles between first character on line and first non- whitespace character on line.
 * <li>INSERT key toggles between insert and overwrite modes.
 * <li>Ctrl+DELETE key deletes all text between the caret and the end of the current line.
 * <li>Ctrl+Shift+Up and Ctrl+Shift+Down move the current line up and down, respectively.
 * <li>Ctrl+J joins lines.
 * <li>Ctrl+Z is undo and Ctrl+Y is redo.
 * <li>Ctrl+Up and Ctrl+Down shift the visible area of the text area up and down one line,
 * respectively.
...
public class RTADefaultInputMap extends InputMap {
	...
	public RTADefaultInputMap() {
		 ...
		 * RTextAreaEditorKit.rtaEndRecordingMacroAction); put(KeyStroke.getKeyStroke(KeyEvent.VK_M,
		 * defaultModifier|shift), RTextAreaEditorKit.rtaPlaybackLastMacroAction);
		 */
		
	}
	
	/**
	 * Returns the default modifier key for a system. For example, on Windows this would be the CTRL
	 * key (<code>InputEvent.CTRL_MASK</code>).
	 *
