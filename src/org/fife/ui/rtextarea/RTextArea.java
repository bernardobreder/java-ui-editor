/*
 * 11/14/2003 RTextArea.java - An extension of JTextArea that adds many features. This library is
 ...
 * </ul>
 * NOTE: If the background for an <code>RTextArea</code> is set to a color, its opaque property is
 * set to <code>true</code> for performance reasons. If the background is set to an image, then the
 * opaque property is set to <code>false</code>. This slows things down a little, but if it didn't
 * happen then we would see garbage on-screen when the user scrolled through a document using the
 * arrow keys (not the page-up/down keys though). You should never have to set the opaque property
 * yourself; it is always done for you.
 *
...
public class RTextArea extends RTextAreaBase implements Printable {
	...
	/**
	 * Can return tool tips for this text area. Subclasses can install a supplier as a means of
	 * adding custom tool tips without subclassing <tt>RTextArea</tt>. {@link #getToolTipText()}
	...
	/**
	 * Adds an action event to the current macro. This shouldn't be called directly, as it is called
	 ...
	 * <p>
	 * Using this method should be done with great care. You should probably wrap the call to
	 * <tt>endAtomicEdit()</tt> in a <tt>finally</tt> block:
	 *
	 * <pre>
	...
	/**
	 * Begins recording a macro. After this method is called, all input/caret events, etc. are
	 * recorded until <code>endMacroRecording</code> is called. If this method is called but the
	 * text component is already recording a macro, nothing happens (but the macro keeps recording).
	 *
	...
	/**
	 * Configures the popup menu for this text area. This method is called right before it is
	 * displayed, so a hosting application can do any custom configuration (configuring actions,
	 * adding/removing items, etc.).
	 * <p>
	 * The default implementation does nothing.
	 * <p>
	 * If you set the popup menu via {@link #setPopupMenu(JPopupMenu)}, you will want to override
	 * this method, especially if you removed any of the menu items in the default popup menu.
	 *
	 ...
	 * explicitly given. A new instance of RDocument is returned.
	 *
	...
	/**
	 * Creates the right-click popup menu. Subclasses can override this method to replace or augment
	 * the popup menu returned.
	 *
	...
	/**
	 * Creates the actions used in the popup menu and retrievable by {@link #getAction(int)}. TODO:
	 * Remove these horrible hacks and move localizing of actions into the editor kits, where it
	 * should be! The context menu should contain actions from the editor kits.
	 */
	...
	/**
	 * Removes all undoable edits from this document's undo manager. This method also makes the
	 * undo/redo actions disabled.
	 */
	/*
	 * NOTE: For some reason, it appears I have to create an entirely new <code>undoManager</code>
	 * for undo/redo to continue functioning properly; if I don't, it only ever lets you do one
	 * undo. Not too sure why this is...
	 */
	...
	/**
	 * Ends recording a macro. If this method is called but the text component is not recording a
	...
	/**
	 * Provides a way to gain access to the editor actions on the right-click popup menu. This way
	 * you can make toolbar/menu bar items use the actual actions used by all <code>RTextArea</code>
	 * s, so that icons stay synchronized and you don't have to worry about enabling/disabling them
	 * yourself.
	 * <p>
	 * Keep in mind that these actions are shared across all instances of <code>RTextArea</code>, so
	 * a change to any action returned by this method is global across all <code>RTextArea</code>
	 * editors in your application.
	 *
	 * @param action The action to retrieve, such as {@link #CUT_ACTION}. If the action name is
	...
	/**
	 * Returns the macro currently stored in this <code>RTextArea</code>. Since macros are shared,
	 * all <code>RTextArea</code>s in the currently- running application are using this macro.
	 *
	...
	/**
	 * Returns the maximum ascent of all fonts used in this text area. In the case of a standard
	 * <code>RTextArea</code>, this is simply the ascent of the current font.
	 * <p>
	...
	/**
	 * Returns the tooltip to display for a mouse event at the given location. This method is
	 * overridden to check for a {@link ToolTipSupplier}; if there is one installed, it is queried
	 * for tool tip text before using the super class's implementation of this method.
	 *
	...
	/**
	 * Does the actual dirty-work of replacing the selected text in this text area (i.e., in its
	 * document). This method provides a hook for subclasses to handle this in a different way.
	 *
	...
	/**
	 * Marks all ranges specified with the "mark all" highlighter. Typically, this method is called
	 * indirectly from {@link SearchEngine} when doing a fine or replace operation.
	 * <p>
	 ...
	 * <code>Document</code>, thus requiring us to re-attach our Undo manager. With this version we
	 ...
	 * <p>
	 * This method is overridden so that our Undo manager remembers it as a single operation (it has
	 * trouble with this, especially for <code>RSyntaxTextArea</code> and the "auto-indent"
	 * feature).
	 *
	...
	/**
	 * Replaces all instances of the tab character in <code>text</code> with the number of spaces
	 * equivalent to a tab in this text area.
	 * <p>
	 * This method should only be called from thread-safe methods, such as
	 * {@link #replaceSelection(String)}.
	 *
	 * @param text The <code>java.lang.String</code> in which to replace tabs with spaces. This has
	 *            already been verified to have at least one tab character in it.
	 * @param docOffs The offset in the document at which the text is being inserted.
	 * @param firstTab The offset into <code>text</code> of the first tab. Assumed to be &gt;= 0.
	 * @return A <code>String</code> just like <code>text</code>, but with spaces instead of tabs.
	 */
	 ...
	 * {@link ConfigurableCaret}s (which is used by default), or a subclass, since they know how to
	 * render themselves differently when the user toggles between insert and overwrite modes.
	 *
	 * @param caret The caret to use. If this is not an instance of <code>ConfigurableCaret</code>,
	 ...
	 * @throws IllegalArgumentException If the document is not an instance of {@link RDocument}.
	 */
	@Override
	...
	/**
	 * Sets the path in which to find images to associate with the editor's actions. The path MUST
	 * contain the following images (with the appropriate extension as defined by the icon group):<br>
	 * <ul>
	 * <li>cut</li>
	 * <li>copy</li>
	 * <li>paste</li>
	 ...
	 * <p>
	 * If you set the popup menu with this method, you'll want to consider also overriding
	 * {@link #configurePopupMenu(JPopupMenu)}, especially if you removed any of the default menu
	 * items.
	 *
	 * @param popupMenu The popup menu. If this is <code>null</code>, no popup menu will be
	 *            displayed.
	 * @see #getPopupMenu()
	 * @see #configurePopupMenu(JPopupMenu)
	 */
	...
	/**
	 * Sets the text last selected/Ctrl+K'd in an <code>RTextArea</code>. This text will be searched
	 * for in subsequent Ctrl+K/Ctrl+Shift+K actions (Cmd+K on OS X).
	 * <p>
	 * Since the selected occurrence actions are built into RTextArea, applications usually do not
	 * have to call this method directly, but can choose to do so if they wish (for example, if they
	 * wish to set this value when the user does a search via a Find dialog).
	 *
	...
	/**
	 * Sets the text mode for this editor pane. If the currently installed caret is an instance of
	 * {@link ConfigurableCaret}, it will be automatically updated to render itself appropriately
	 * for the new text mode.
	 *
	 * @param mode Either {@link #INSERT_MODE} or {@link #OVERWRITE_MODE}.
	 * @see #getTextMode()
	 */
	 ...
	 *
	 * @param supplier The new tool tip supplier, or <code>null</code> if there is to be no
	 *            supplier.
	 * @see #getToolTipSupplier()
	 */
	...
	/**
	 * Sets the UI used by this text area. This is overridden so only the right-click popup menu's
	 * UI is updated. The look and feel of an <code>RTextArea</code> is independent of the Java Look
	 * and Feel, and so this method does not change the text area itself. Subclasses (such as
	 * <code>RSyntaxTextArea</code> can call <code>setRTextAreaUI</code> if they wish to install a
	 * new UI.
	 *
	...
	/**
	 * Modified from <code>MutableCaretEvent</code> in <code>JTextComponent</code> so that mouse
	 * events get fired when the user is selecting text with the mouse as well. This class also
	 * displays the popup menu when the user right-clicks in the text area.
	 */
	protected class RTextAreaMutableCaretEvent extends RTAMouseListener {
		...
		/**
		 * Shows a popup menu with cut, copy, paste, etc. options if the user clicked the right
		 * button.
		 *
