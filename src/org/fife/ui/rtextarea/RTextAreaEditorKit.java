...
/**
 * An extension of <code>DefaultEditorKit</code> that adds functionality found in
 * <code>RTextArea</code>.
 *
...
public class RTextAreaEditorKit extends DefaultEditorKit {
	...
	/**
	 * The name of the action taken to delete the remainder of the line (from the caret position to
	 * the end of the line).
	 */
	public static final String rtaDeleteRestOfLineAction = "RTA.DeleteRestOfLineAction";
	
	/**
	 * The name of the action that completes the word at the caret position with the last word in
	 * the document that starts with the text up to the caret.
	 */
	...
	/**
	 * The name of the action to scroll the text area down one line without changing the caret's
	 * position.
	 */
	public static final String rtaScrollDownAction = "RTA.ScrollDownAction";
	
	/**
	 * The name of the action to scroll the text area up one line without changing the caret's
	 * position.
	 */
	...
	/**
	 * The name of the action taken when the user hits the Insert key (thus toggling between insert
	 * and overwrite modes).
	 */
	...
	/**
	 * The actions that <code>RTextAreaEditorKit</code> adds to those of the default editor kit.
	 */
	...
	/**
	 * Inserts content from the given stream, which will be treated as plain text. This method is
	 * overridden merely so we can increase the number of characters read at a time.
	 *
	...
	/**
	 * Toggles the position of the caret between the beginning of the line, and the first
	...
	/**
	 * The action to use when no actions in the input/action map meet the key pressed. This is
	 * actually called from the keymap I believe.
	 */
	...
	/**
	 * Finds the most recent word in the document that matches the "word" up to the current caret
	 * position, and auto-completes the rest. Repeatedly calling this action at the same location in
	 * the document goes one match back each time it is called.
	 */
	...
	public static class InsertBreakAction extends RecordableTextAction {
		...
		/*
		 * Overridden for Sun bug 4515750. Sun fixed this in a more complicated way, but I'm not
		 * sure why. See BasicTextUI#getActionMap() and BasicTextUI.TextActionWrapper.
		 */
		@Override
	...
	/**
	 * Places a tab character into the document. If there is a selection, it is removed before the
	 * tab is added.
	 */
	...
	/**
	 * Action to move the selection and/or caret. Constructor indicates direction to use.
	 */
	public static class NextVisualPositionAction extends RecordableTextAction {
		...
		public void actionPerformedImpl(ActionEvent e, RTextArea textArea) {
			...
			/*
			 * Move to the beginning/end of selection on a "non-shifted" left- or right-keypress. We
			 * shouldn't have to worry about navigation filters as, if one is being used, it let us
			 * get to that position before.
			 */
			if (!select) {
				switch (direction) {
	...
	/**
	 * Scrolls up/down vertically. The select version of this action extends the selection, instead
	 * of simply moving the caret.
	 */
