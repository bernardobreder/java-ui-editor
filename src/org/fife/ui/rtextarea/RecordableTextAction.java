...
/**
 * The base action used by the actions defined in {@link RTextAreaEditorKit}. This action is what
 * allows instances of <code>RTextArea</code> to record keystrokes into "macros;" if an action is
 * recordable and occurs while the user is recording a macro, it adds itself to the
 * currently-being-recorded macro.
 *
...
public abstract class RecordableTextAction extends TextAction {
	...
	/**
	 * This method is final so that you cannot override it and mess up the macro-recording part of
	 * it. The actual meat of the action is in <code>actionPerformedImpl</code>.
	 *
	...
	public final void actionPerformed(ActionEvent e) {
		...
		if (textComponent instanceof RTextArea) {
			...
			if (RTextArea.isRecordingMacro() && isRecordable()) {
				...
				if (!DefaultEditorKit.defaultKeyTypedAction.equals(macroID) || ((mod & ActionEvent.ALT_MASK) == 0 && (mod & ActionEvent.CTRL_MASK) == 0 && (mod & ActionEvent.META_MASK) == 0)) {
	...
	/**
	 * Returns the identifier for this macro. This method makes it so that you can create an
	 * instance of the <code>RTextAreaEditorKit.CutAction</code> action, for example, rename it to
	 * "Remove", and it will still be recorded as a "cut" action. Subclasses should return a unique
	 * string from this method; preferably the name of the action.
	 * <p>
	 * If you subclass a <code>RecordableTextAction</code>, you should NOT override this method; if
	 * you do, the action may not be properly recorded in a macro.
	 *
	 ...
	 *            <code>.Mnemonic</code>", and "<code>.Desc</code>" will be appended to create the
